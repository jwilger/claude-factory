namespace PresentAskHumanGateWithContextHeader

-- EMC-DIGEST: workflow:name=Present Ask-Human Gate with Context Header;slug=present-ask-human-gate-with-context;description=Workflow covering how the kernel enriches an ask_human gate step with a context header before presenting it to the operator: assembles work item metadata, phase history, and the question into a structured prompt so the human can make an informed decision without needing to look up context elsewhere.;slices=assemble-human-gate-context-header|Assemble Context Header for Human Gate|state_view|Given ask_human step is scheduled for a work item, When cf_next_step is called, Then context header assembled from work_item description, current phase, relevant prior events, and any escalation notes; returned as context_header field in the ask_human step payload|entry,present-human-decision-request|Present Human Decision Request to Operator|state_change|Given context header assembled, When conductor displays the ask_human gate to the operator with the question and context header, Then HumanDecisionRequested event emitted with work_item_id, question, context_header, and requested_at_ms|main,record-human-decision|Record Human Decision|state_change|Given HumanDecisionRequested, When operator provides their decision and conductor calls cf_decide, Then HumanDecisionRecorded event emitted with work_item_id, decision, and decided_at_ms; work item advances to the appropriate next state|main;transitions=assemble-human-gate-context-header->present-human-decision-request:event:AskHumanStepReady::{ work_item_id: Uuid, question: String, context_header: String },present-human-decision-request->record-human-decision:event:HumanDecisionRequested::{ work_item_id: Uuid, question: String, context_header: String, requested_at_ms: u64 };outcomes=;command_errors=;owned_definitions=;transition_evidences=;entry_lifecycle_required=false;entry_lifecycle_states=
-- EMC generated Lean4 business workflow model.
def workflowName := "Present Ask-Human Gate with Context Header"

def workflowSlug := "present-ask-human-gate-with-context"

def workflowDescription := "Workflow covering how the kernel enriches an ask_human gate step with a context header before presenting it to the operator: assembles work item metadata, phase history, and the question into a structured prompt so the human can make an informed decision without needing to look up context elsewhere."

structure WorkflowSlice where
  slug : String

def workflowSlices : List WorkflowSlice := [{ slug := "assemble-human-gate-context-header" },{ slug := "present-human-decision-request" },{ slug := "record-human-decision" }]
...
def workflowSliceDetails : List WorkflowSliceDetail := [{ slug := "assemble-human-gate-context-header", name := "Assemble Context Header for Human Gate", kind := SliceKindName.stateView, description := "Given ask_human step is scheduled for a work item, When cf_next_step is called, Then context header assembled from work_item description, current phase, relevant prior events, and any escalation notes; returned as context_header field in the ask_human step payload" },{ slug := "present-human-decision-request", name := "Present Human Decision Request to Operator", kind := SliceKindName.stateChange, description := "Given context header assembled, When conductor displays the ask_human gate to the operator with the question and context header, Then HumanDecisionRequested event emitted with work_item_id, question, context_header, and requested_at_ms" },{ slug := "record-human-decision", name := "Record Human Decision", kind := SliceKindName.stateChange, description := "Given HumanDecisionRequested, When operator provides their decision and conductor calls cf_decide, Then HumanDecisionRecorded event emitted with work_item_id, decision, and decided_at_ms; work item advances to the appropriate next state" }]

def workflowSliceModules : List WorkflowSliceModule := [{ slice := "assemble-human-gate-context-header", formalModule := "AssembleContextHeaderForHumanGate" },{ slice := "present-human-decision-request", formalModule := "PresentHumanDecisionRequestToOperator" },{ slice := "record-human-decision", formalModule := "RecordHumanDecision" }]
...
def workflowTransitions : List WorkflowTransition := [{ source := "assemble-human-gate-context-header", target := "present-human-decision-request", kind := WorkflowTransitionKind.event, trigger := "AskHumanStepReady", sourceControl := "", targetView := "", rationale := "", payloadContract := "{ work_item_id: Uuid, question: String, context_header: String }" },{ source := "present-human-decision-request", target := "record-human-decision", kind := WorkflowTransitionKind.event, trigger := "HumanDecisionRequested", sourceControl := "", targetView := "", rationale := "", payloadContract := "{ work_item_id: Uuid, question: String, context_header: String, requested_at_ms: u64 }" }]
...
def workflowStepRelationships : List WorkflowStepRelationship := [{ step := "assemble-human-gate-context-header", relationship := WorkflowStepRelationshipName.entry },{ step := "present-human-decision-request", relationship := WorkflowStepRelationshipName.main },{ step := "record-human-decision", relationship := WorkflowStepRelationshipName.main }]
...
theorem workflowIdentityIsStable : workflowName = "Present Ask-Human Gate with Context Header" := rfl
...
end PresentAskHumanGateWithContextHeader
