namespace KernelOpensInteractiveGateAndAcquiresLock

-- EMC-DIGEST: workflow:name=Kernel Opens Interactive Gate and Acquires Lock;slug=kernel-opens-interactive-gate-acquires-lock;description=Workflow covering gate serialisation in concurrent phase runs: before opening any interactive gate (gate_review or ask_human), the kernel acquires a per-work-item lock to prevent concurrent gate openings; the lock is released when the verdict is recorded.;slices=acquire-gate-lock-before-opening|Acquire Gate Lock Before Opening Interactive Gate|state_change|Given gate_review or ask_human step is scheduled and no gate lock is currently held for this work item, When kernel prepares to open the gate, Then GateLockAcquired event emitted with work_item_id, gate_kind, and acquired_at_ms; concurrent gate attempts for the same item are rejected|entry,open-interactive-gate-with-lock-held|Open Interactive Gate with Lock Held|state_change|Given GateLockAcquired, When reviewer agent is spawned or human question is presented, Then InteractiveGateOpened event emitted with work_item_id, gate_kind, and opened_at_ms confirming the gate is active|main,release-gate-lock-on-verdict|Release Gate Lock After Verdict Recorded|automation|Given GateApproved or GateVetoed event emitted for the work item, When verdict is recorded and the gate closes, Then GateLockReleased event emitted with work_item_id and released_at_ms; the next gate for any work item can now be opened|main;transitions=acquire-gate-lock-before-opening->open-interactive-gate-with-lock-held:event:GateLockAcquired::{ work_item_id: Uuid, gate_kind: String, acquired_at_ms: u64 },open-interactive-gate-with-lock-held->release-gate-lock-on-verdict:event:InteractiveGateOpened::{ work_item_id: Uuid, gate_kind: String, opened_at_ms: u64 };outcomes=;command_errors=;owned_definitions=;transition_evidences=;entry_lifecycle_required=false;entry_lifecycle_states=
-- EMC generated Lean4 business workflow model.
def workflowName := "Kernel Opens Interactive Gate and Acquires Lock"

def workflowSlug := "kernel-opens-interactive-gate-acquires-lock"

def workflowDescription := "Workflow covering gate serialisation in concurrent phase runs: before opening any interactive gate (gate_review or ask_human), the kernel acquires a per-work-item lock to prevent concurrent gate openings; the lock is released when the verdict is recorded."

structure WorkflowSlice where
  slug : String

def workflowSlices : List WorkflowSlice := [{ slug := "acquire-gate-lock-before-opening" },{ slug := "open-interactive-gate-with-lock-held" },{ slug := "release-gate-lock-on-verdict" }]
...
def workflowSliceDetails : List WorkflowSliceDetail := [{ slug := "acquire-gate-lock-before-opening", name := "Acquire Gate Lock Before Opening Interactive Gate", kind := SliceKindName.stateChange, description := "Given gate_review or ask_human step is scheduled and no gate lock is currently held for this work item, When kernel prepares to open the gate, Then GateLockAcquired event emitted with work_item_id, gate_kind, and acquired_at_ms; concurrent gate attempts for the same item are rejected" },{ slug := "open-interactive-gate-with-lock-held", name := "Open Interactive Gate with Lock Held", kind := SliceKindName.stateChange, description := "Given GateLockAcquired, When reviewer agent is spawned or human question is presented, Then InteractiveGateOpened event emitted with work_item_id, gate_kind, and opened_at_ms confirming the gate is active" },{ slug := "release-gate-lock-on-verdict", name := "Release Gate Lock After Verdict Recorded", kind := SliceKindName.automation, description := "Given GateApproved or GateVetoed event emitted for the work item, When verdict is recorded and the gate closes, Then GateLockReleased event emitted with work_item_id and released_at_ms; the next gate for any work item can now be opened" }]

def workflowSliceModules : List WorkflowSliceModule := [{ slice := "acquire-gate-lock-before-opening", formalModule := "AcquireGateLockBeforeOpeningInteractiveGate" },{ slice := "open-interactive-gate-with-lock-held", formalModule := "OpenInteractiveGateWithLockHeld" },{ slice := "release-gate-lock-on-verdict", formalModule := "ReleaseGateLockAfterVerdictRecorded" }]
...
def workflowTransitions : List WorkflowTransition := [{ source := "acquire-gate-lock-before-opening", target := "open-interactive-gate-with-lock-held", kind := WorkflowTransitionKind.event, trigger := "GateLockAcquired", sourceControl := "", targetView := "", rationale := "", payloadContract := "{ work_item_id: Uuid, gate_kind: String, acquired_at_ms: u64 }" },{ source := "open-interactive-gate-with-lock-held", target := "release-gate-lock-on-verdict", kind := WorkflowTransitionKind.event, trigger := "InteractiveGateOpened", sourceControl := "", targetView := "", rationale := "", payloadContract := "{ work_item_id: Uuid, gate_kind: String, opened_at_ms: u64 }" }]
...
def workflowStepRelationships : List WorkflowStepRelationship := [{ step := "acquire-gate-lock-before-opening", relationship := WorkflowStepRelationshipName.entry },{ step := "open-interactive-gate-with-lock-held", relationship := WorkflowStepRelationshipName.main },{ step := "release-gate-lock-on-verdict", relationship := WorkflowStepRelationshipName.main }]
...
theorem workflowIdentityIsStable : workflowName = "Kernel Opens Interactive Gate and Acquires Lock" := rfl
...
end KernelOpensInteractiveGateAndAcquiresLock
