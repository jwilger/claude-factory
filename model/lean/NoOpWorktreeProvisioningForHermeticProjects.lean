namespace NoOpWorktreeProvisioningForHermeticProjects

-- EMC-DIGEST: workflow:name=No-Op Worktree Provisioning for Hermetic Projects;slug=noop-hermetic-projects;description=Workflow covering the bypass path when worktree isolation is disabled: hermetic projects (where all agents share a single checkout) skip git worktree creation, emitting WorktreeProvisionSkipped so the TDD state machine advances without a real worktree_path.;slices=detect-hermetic-project-flag|Detect Hermetic Project Configuration|state_view|Given work item claimed for development, When kernel checks worktree provisioning config, Then return Hermetic or IsolationRequired outcome based on whether worktree_isolation is disabled in project config|entry,emit-worktree-provision-skipped|Emit WorktreeProvisionSkipped for Hermetic Project|state_change|Given Hermetic outcome detected, When worktree provisioning would normally run, Then WorktreeProvisionSkipped event emitted with work_item_id and reason=hermetic_project so the TDD state machine can proceed using the shared checkout path|main,expose-hermetic-skip-in-status|Expose Hermetic Skip in cf_status|state_view|Given WorktreeProvisionSkipped event recorded, When cf_status called, Then worktree entry for this work item shows isolation=hermetic_skip and project_root as the effective working directory instead of a dedicated worktree_path|main;transitions=detect-hermetic-project-flag->emit-worktree-provision-skipped:outcome:Hermetic::,emit-worktree-provision-skipped->expose-hermetic-skip-in-status:event:WorktreeProvisionSkipped::{ work_item_id: Uuid, reason: String };outcomes=;command_errors=;owned_definitions=;transition_evidences=;entry_lifecycle_required=false;entry_lifecycle_states=
-- EMC generated Lean4 business workflow model.
def workflowName := "No-Op Worktree Provisioning for Hermetic Projects"

def workflowSlug := "noop-hermetic-projects"

def workflowDescription := "Workflow covering the bypass path when worktree isolation is disabled: hermetic projects (where all agents share a single checkout) skip git worktree creation, emitting WorktreeProvisionSkipped so the TDD state machine advances without a real worktree_path."

structure WorkflowSlice where
  slug : String

def workflowSlices : List WorkflowSlice := [{ slug := "detect-hermetic-project-flag" },{ slug := "emit-worktree-provision-skipped" },{ slug := "expose-hermetic-skip-in-status" }]
...
def workflowSliceDetails : List WorkflowSliceDetail := [{ slug := "detect-hermetic-project-flag", name := "Detect Hermetic Project Configuration", kind := SliceKindName.stateView, description := "Given work item claimed for development, When kernel checks worktree provisioning config, Then return Hermetic or IsolationRequired outcome based on whether worktree_isolation is disabled in project config" },{ slug := "emit-worktree-provision-skipped", name := "Emit WorktreeProvisionSkipped for Hermetic Project", kind := SliceKindName.stateChange, description := "Given Hermetic outcome detected, When worktree provisioning would normally run, Then WorktreeProvisionSkipped event emitted with work_item_id and reason=hermetic_project so the TDD state machine can proceed using the shared checkout path" },{ slug := "expose-hermetic-skip-in-status", name := "Expose Hermetic Skip in cf_status", kind := SliceKindName.stateView, description := "Given WorktreeProvisionSkipped event recorded, When cf_status called, Then worktree entry for this work item shows isolation=hermetic_skip and project_root as the effective working directory instead of a dedicated worktree_path" }]

def workflowSliceModules : List WorkflowSliceModule := [{ slice := "detect-hermetic-project-flag", formalModule := "DetectHermeticProjectConfiguration" },{ slice := "emit-worktree-provision-skipped", formalModule := "EmitWorktreeProvisionSkippedForHermeticProject" },{ slice := "expose-hermetic-skip-in-status", formalModule := "ExposeHermeticSkipInCfStatus" }]
...
def workflowTransitions : List WorkflowTransition := [{ source := "detect-hermetic-project-flag", target := "emit-worktree-provision-skipped", kind := WorkflowTransitionKind.outcome, trigger := "Hermetic", sourceControl := "", targetView := "", rationale := "", payloadContract := "" },{ source := "emit-worktree-provision-skipped", target := "expose-hermetic-skip-in-status", kind := WorkflowTransitionKind.event, trigger := "WorktreeProvisionSkipped", sourceControl := "", targetView := "", rationale := "", payloadContract := "{ work_item_id: Uuid, reason: String }" }]
...
def workflowStepRelationships : List WorkflowStepRelationship := [{ step := "detect-hermetic-project-flag", relationship := WorkflowStepRelationshipName.entry },{ step := "emit-worktree-provision-skipped", relationship := WorkflowStepRelationshipName.main },{ step := "expose-hermetic-skip-in-status", relationship := WorkflowStepRelationshipName.main }]
...
theorem workflowIdentityIsStable : workflowName = "No-Op Worktree Provisioning for Hermetic Projects" := rfl
...
end NoOpWorktreeProvisioningForHermeticProjects
