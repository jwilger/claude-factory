namespace TearDownWorktreeAndCleanSecrets

-- EMC-DIGEST: workflow:name=Tear Down Worktree and Clean Secrets;slug=tear-down-worktree-clean-secrets;description=Workflow covering worktree cleanup when a work item reaches a terminal state (Done or Abandoned): an optional teardown hook runs first to remove secrets and temp files, then git worktree remove is executed, and WorktreeTornDown is emitted so the inventory is updated.;slices=trigger-teardown-on-terminal-state|Trigger Teardown on Terminal Work Item State|automation|Given work item transitions to Done or Abandoned state and a WorktreeProvisioningComplete record exists for that work item, When terminal event is emitted, Then TeardownWorktree command emitted with work_item_id and worktree_path from the provisioning record|entry,run-teardown-hook-clean-secrets|Run Teardown Hook to Clean Secrets|state_change|Given TeardownWorktree command and optional teardown hook script present, When hook executes inside the worktree directory, Then TeardownHookCompleted event emitted with work_item_id, exit_code, and stderr_tail; non-zero exit is recorded but does not block worktree removal|main,remove-git-worktree|Remove Git Worktree|state_change|Given TeardownHookCompleted (or hook absent), When git worktree remove executes for the worktree_path, Then WorktreeTornDown event emitted with work_item_id, worktree_path, branch_name, and torn_down_at_ms; inventory entry removed from cf_status|main;transitions=trigger-teardown-on-terminal-state->run-teardown-hook-clean-secrets:command:TeardownWorktree::,run-teardown-hook-clean-secrets->remove-git-worktree:event:TeardownHookCompleted::{ work_item_id: Uuid, exit_code: i32, stderr_tail: String };outcomes=;command_errors=;owned_definitions=;transition_evidences=;entry_lifecycle_required=false;entry_lifecycle_states=
-- EMC generated Lean4 business workflow model.
def workflowName := "Tear Down Worktree and Clean Secrets"

def workflowSlug := "tear-down-worktree-clean-secrets"

def workflowDescription := "Workflow covering worktree cleanup when a work item reaches a terminal state (Done or Abandoned): an optional teardown hook runs first to remove secrets and temp files, then git worktree remove is executed, and WorktreeTornDown is emitted so the inventory is updated."

structure WorkflowSlice where
  slug : String

def workflowSlices : List WorkflowSlice := [{ slug := "trigger-teardown-on-terminal-state" },{ slug := "run-teardown-hook-clean-secrets" },{ slug := "remove-git-worktree" }]
...
def workflowSliceDetails : List WorkflowSliceDetail := [{ slug := "trigger-teardown-on-terminal-state", name := "Trigger Teardown on Terminal Work Item State", kind := SliceKindName.automation, description := "Given work item transitions to Done or Abandoned state and a WorktreeProvisioningComplete record exists for that work item, When terminal event is emitted, Then TeardownWorktree command emitted with work_item_id and worktree_path from the provisioning record" },{ slug := "run-teardown-hook-clean-secrets", name := "Run Teardown Hook to Clean Secrets", kind := SliceKindName.stateChange, description := "Given TeardownWorktree command and optional teardown hook script present, When hook executes inside the worktree directory, Then TeardownHookCompleted event emitted with work_item_id, exit_code, and stderr_tail; non-zero exit is recorded but does not block worktree removal" },{ slug := "remove-git-worktree", name := "Remove Git Worktree", kind := SliceKindName.stateChange, description := "Given TeardownHookCompleted (or hook absent), When git worktree remove executes for the worktree_path, Then WorktreeTornDown event emitted with work_item_id, worktree_path, branch_name, and torn_down_at_ms; inventory entry removed from cf_status" }]

def workflowSliceModules : List WorkflowSliceModule := [{ slice := "trigger-teardown-on-terminal-state", formalModule := "TriggerTeardownOnTerminalWorkItemState" },{ slice := "run-teardown-hook-clean-secrets", formalModule := "RunTeardownHookToCleanSecrets" },{ slice := "remove-git-worktree", formalModule := "RemoveGitWorktree" }]
...
def workflowTransitions : List WorkflowTransition := [{ source := "trigger-teardown-on-terminal-state", target := "run-teardown-hook-clean-secrets", kind := WorkflowTransitionKind.command, trigger := "TeardownWorktree", sourceControl := "", targetView := "", rationale := "", payloadContract := "" },{ source := "run-teardown-hook-clean-secrets", target := "remove-git-worktree", kind := WorkflowTransitionKind.event, trigger := "TeardownHookCompleted", sourceControl := "", targetView := "", rationale := "", payloadContract := "{ work_item_id: Uuid, exit_code: i32, stderr_tail: String }" }]
...
def workflowStepRelationships : List WorkflowStepRelationship := [{ step := "trigger-teardown-on-terminal-state", relationship := WorkflowStepRelationshipName.entry },{ step := "run-teardown-hook-clean-secrets", relationship := WorkflowStepRelationshipName.main },{ step := "remove-git-worktree", relationship := WorkflowStepRelationshipName.main }]
...
theorem workflowIdentityIsStable : workflowName = "Tear Down Worktree and Clean Secrets" := rfl
...
end TearDownWorktreeAndCleanSecrets
