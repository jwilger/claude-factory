namespace RunAgentInReadyEnvironment

-- EMC-DIGEST: workflow:name=Run Agent in Ready Environment;slug=run-agent-in-ready-environment;description=Workflow covering agent execution within a fully provisioned worktree: once setup is complete, the kernel injects worktree_path into the spawn_agent step context, the agent executes TDD work inside that isolated directory, and the conductor submits the result via cf_submit.;slices=inject-worktree-context-into-step|Inject Worktree Context into Step Prompt|automation|Given WorktreeProvisioningComplete recorded for a work item, When cf_next_step generates the spawn_agent step, Then worktree_path and branch_name are appended to the step prompt so the agent knows exactly where to read and write code|entry,agent-executes-tdd-work-in-worktree|Agent Executes TDD Work in Worktree|state_change|Given spawn_agent step with worktree_path in prompt, When conductor runs the agent inside the worktree directory, Then AgentWorkCompleted event emitted with work_item_id, phase, and result payload ready for cf_submit|main,submit-result-advancing-tdd-state|Submit Result Advancing TDD State Machine|state_change|Given AgentWorkCompleted event with result payload, When conductor calls cf_submit with the result, Then TDD state machine advances to the next phase (test_review, implement, or run_check) and the work item's phase is updated in the read model|main;transitions=inject-worktree-context-into-step->agent-executes-tdd-work-in-worktree:command:SpawnAgentWithContext::,agent-executes-tdd-work-in-worktree->submit-result-advancing-tdd-state:event:AgentWorkCompleted::{ work_item_id: Uuid, phase: String, result: serde_json::Value };outcomes=;command_errors=;owned_definitions=;transition_evidences=;entry_lifecycle_required=false;entry_lifecycle_states=
-- EMC generated Lean4 business workflow model.
def workflowName := "Run Agent in Ready Environment"

def workflowSlug := "run-agent-in-ready-environment"

def workflowDescription := "Workflow covering agent execution within a fully provisioned worktree: once setup is complete, the kernel injects worktree_path into the spawn_agent step context, the agent executes TDD work inside that isolated directory, and the conductor submits the result via cf_submit."

structure WorkflowSlice where
  slug : String

def workflowSlices : List WorkflowSlice := [{ slug := "inject-worktree-context-into-step" },{ slug := "agent-executes-tdd-work-in-worktree" },{ slug := "submit-result-advancing-tdd-state" }]
...
def workflowSliceDetails : List WorkflowSliceDetail := [{ slug := "inject-worktree-context-into-step", name := "Inject Worktree Context into Step Prompt", kind := SliceKindName.automation, description := "Given WorktreeProvisioningComplete recorded for a work item, When cf_next_step generates the spawn_agent step, Then worktree_path and branch_name are appended to the step prompt so the agent knows exactly where to read and write code" },{ slug := "agent-executes-tdd-work-in-worktree", name := "Agent Executes TDD Work in Worktree", kind := SliceKindName.stateChange, description := "Given spawn_agent step with worktree_path in prompt, When conductor runs the agent inside the worktree directory, Then AgentWorkCompleted event emitted with work_item_id, phase, and result payload ready for cf_submit" },{ slug := "submit-result-advancing-tdd-state", name := "Submit Result Advancing TDD State Machine", kind := SliceKindName.stateChange, description := "Given AgentWorkCompleted event with result payload, When conductor calls cf_submit with the result, Then TDD state machine advances to the next phase (test_review, implement, or run_check) and the work item's phase is updated in the read model" }]

def workflowSliceModules : List WorkflowSliceModule := [{ slice := "inject-worktree-context-into-step", formalModule := "InjectWorktreeContextIntoStepPrompt" },{ slice := "agent-executes-tdd-work-in-worktree", formalModule := "AgentExecutesTDDWorkInWorktree" },{ slice := "submit-result-advancing-tdd-state", formalModule := "SubmitResultAdvancingTDDStateMachine" }]
...
def workflowTransitions : List WorkflowTransition := [{ source := "inject-worktree-context-into-step", target := "agent-executes-tdd-work-in-worktree", kind := WorkflowTransitionKind.command, trigger := "SpawnAgentWithContext", sourceControl := "", targetView := "", rationale := "", payloadContract := "" },{ source := "agent-executes-tdd-work-in-worktree", target := "submit-result-advancing-tdd-state", kind := WorkflowTransitionKind.event, trigger := "AgentWorkCompleted", sourceControl := "", targetView := "", rationale := "", payloadContract := "{ work_item_id: Uuid, phase: String, result: serde_json::Value }" }]
...
def workflowStepRelationships : List WorkflowStepRelationship := [{ step := "inject-worktree-context-into-step", relationship := WorkflowStepRelationshipName.entry },{ step := "agent-executes-tdd-work-in-worktree", relationship := WorkflowStepRelationshipName.main },{ step := "submit-result-advancing-tdd-state", relationship := WorkflowStepRelationshipName.main }]
...
theorem workflowIdentityIsStable : workflowName = "Run Agent in Ready Environment" := rfl
...
end RunAgentInReadyEnvironment
