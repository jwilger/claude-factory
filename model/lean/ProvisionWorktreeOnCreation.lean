namespace ProvisionWorktreeOnCreation

-- EMC-DIGEST: workflow:name=Provision Worktree on Creation;slug=provision-worktree-on-creation;description=Workflow covering automatic git worktree provisioning when a development work item is claimed: the kernel runs git worktree add, emits WorktreeProvisioned, and makes the worktree_path available in the step context so the spawned agent writes code in an isolated directory.;slices=emit-provision-worktree-command|Emit ProvisionWorktree Command on Claim|automation|Given development work item claimed and worktree config is valid, When cf_claim succeeds, Then ProvisionWorktree command emitted with work_item_id, base_branch, and worktree_path derived from naming_template|entry,create-git-worktree|Create Git Worktree|state_change|Given ProvisionWorktree command received, When git worktree add is executed with the derived branch name and worktree_path, Then WorktreeProvisioned event emitted with work_item_id, worktree_path, branch_name, and provisioned_at_ms|main,expose-worktree-path-in-step-context|Expose Worktree Path in Step Context|state_view|Given WorktreeProvisioned event emitted for a work item, When cf_next_step returns the next step for that work item, Then step context includes worktree_path so the spawned agent writes code in the isolated worktree directory|main;transitions=emit-provision-worktree-command->create-git-worktree:command:ProvisionWorktree::,create-git-worktree->expose-worktree-path-in-step-context:event:WorktreeProvisioned::{ work_item_id: Uuid, worktree_path: PathBuf, branch_name: String, provisioned_at_ms: u64 };outcomes=;command_errors=;owned_definitions=;transition_evidences=;entry_lifecycle_required=false;entry_lifecycle_states=
-- EMC generated Lean4 business workflow model.
def workflowName := "Provision Worktree on Creation"

def workflowSlug := "provision-worktree-on-creation"

def workflowDescription := "Workflow covering automatic git worktree provisioning when a development work item is claimed: the kernel runs git worktree add, emits WorktreeProvisioned, and makes the worktree_path available in the step context so the spawned agent writes code in an isolated directory."

structure WorkflowSlice where
  slug : String

def workflowSlices : List WorkflowSlice := [{ slug := "emit-provision-worktree-command" },{ slug := "create-git-worktree" },{ slug := "expose-worktree-path-in-step-context" }]
...
def workflowSliceDetails : List WorkflowSliceDetail := [{ slug := "emit-provision-worktree-command", name := "Emit ProvisionWorktree Command on Claim", kind := SliceKindName.automation, description := "Given development work item claimed and worktree config is valid, When cf_claim succeeds, Then ProvisionWorktree command emitted with work_item_id, base_branch, and worktree_path derived from naming_template" },{ slug := "create-git-worktree", name := "Create Git Worktree", kind := SliceKindName.stateChange, description := "Given ProvisionWorktree command received, When git worktree add is executed with the derived branch name and worktree_path, Then WorktreeProvisioned event emitted with work_item_id, worktree_path, branch_name, and provisioned_at_ms" },{ slug := "expose-worktree-path-in-step-context", name := "Expose Worktree Path in Step Context", kind := SliceKindName.stateView, description := "Given WorktreeProvisioned event emitted for a work item, When cf_next_step returns the next step for that work item, Then step context includes worktree_path so the spawned agent writes code in the isolated worktree directory" }]

def workflowSliceModules : List WorkflowSliceModule := [{ slice := "emit-provision-worktree-command", formalModule := "EmitProvisionWorktreeCommandOnClaim" },{ slice := "create-git-worktree", formalModule := "CreateGitWorktree" },{ slice := "expose-worktree-path-in-step-context", formalModule := "ExposeWorktreePathInStepContext" }]
...
def workflowTransitions : List WorkflowTransition := [{ source := "emit-provision-worktree-command", target := "create-git-worktree", kind := WorkflowTransitionKind.command, trigger := "ProvisionWorktree", sourceControl := "", targetView := "", rationale := "", payloadContract := "" },{ source := "create-git-worktree", target := "expose-worktree-path-in-step-context", kind := WorkflowTransitionKind.event, trigger := "WorktreeProvisioned", sourceControl := "", targetView := "", rationale := "", payloadContract := "{ work_item_id: Uuid, worktree_path: PathBuf, branch_name: String, provisioned_at_ms: u64 }" }]
...
def workflowStepRelationships : List WorkflowStepRelationship := [{ step := "emit-provision-worktree-command", relationship := WorkflowStepRelationshipName.entry },{ step := "create-git-worktree", relationship := WorkflowStepRelationshipName.main },{ step := "expose-worktree-path-in-step-context", relationship := WorkflowStepRelationshipName.main }]
...
theorem workflowIdentityIsStable : workflowName = "Provision Worktree on Creation" := rfl
...
end ProvisionWorktreeOnCreation
