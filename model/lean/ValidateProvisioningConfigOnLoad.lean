namespace ValidateProvisioningConfigOnLoad

-- EMC-DIGEST: workflow:name=Validate Provisioning Config on Load;slug=validate-provisioning-config-on-load;description=Workflow covering startup validation of the worktree provisioning config: after event replay restores the config, the kernel verifies the base branch exists and the worktree directory is accessible, emitting a validation outcome so operators see config_valid status in cf_status.;slices=restore-worktree-config-from-events|Restore Worktree Config from Event Replay|state_view|Given server replays events on startup, When WorktreeConfigDeclared event is processed, Then current worktree_config read model is populated with base_branch, worktree_dir, and naming_template|entry,validate-config-coherence-on-startup|Validate Config Coherence on Startup|automation|Given worktree_config restored from replay, When server completes startup, Then check that base_branch exists in the git repo and worktree_dir path is accessible; emit ConfigValidationPassed or ConfigValidationFailed with reason|main,expose-config-validity-in-status|Expose Config Validity in cf_status|state_view|Given ConfigValidationPassed or ConfigValidationFailed emitted, When operator calls cf_status, Then worktree_config section includes config_valid boolean and optional validation_error string so operators can diagnose misconfiguration|main;transitions=restore-worktree-config-from-events->validate-config-coherence-on-startup:event:WorktreeConfigDeclared::{ base_branch: String, worktree_dir: PathBuf, naming_template: String },validate-config-coherence-on-startup->expose-config-validity-in-status:outcome:ConfigValidationPassed::,validate-config-coherence-on-startup->expose-config-validity-in-status:outcome:ConfigValidationFailed::;outcomes=;command_errors=;owned_definitions=;transition_evidences=;entry_lifecycle_required=false;entry_lifecycle_states=
-- EMC generated Lean4 business workflow model.
def workflowName := "Validate Provisioning Config on Load"

def workflowSlug := "validate-provisioning-config-on-load"

def workflowDescription := "Workflow covering startup validation of the worktree provisioning config: after event replay restores the config, the kernel verifies the base branch exists and the worktree directory is accessible, emitting a validation outcome so operators see config_valid status in cf_status."

structure WorkflowSlice where
  slug : String

def workflowSlices : List WorkflowSlice := [{ slug := "restore-worktree-config-from-events" },{ slug := "validate-config-coherence-on-startup" },{ slug := "expose-config-validity-in-status" }]
...
def workflowSliceDetails : List WorkflowSliceDetail := [{ slug := "restore-worktree-config-from-events", name := "Restore Worktree Config from Event Replay", kind := SliceKindName.stateView, description := "Given server replays events on startup, When WorktreeConfigDeclared event is processed, Then current worktree_config read model is populated with base_branch, worktree_dir, and naming_template" },{ slug := "validate-config-coherence-on-startup", name := "Validate Config Coherence on Startup", kind := SliceKindName.automation, description := "Given worktree_config restored from replay, When server completes startup, Then check that base_branch exists in the git repo and worktree_dir path is accessible; emit ConfigValidationPassed or ConfigValidationFailed with reason" },{ slug := "expose-config-validity-in-status", name := "Expose Config Validity in cf_status", kind := SliceKindName.stateView, description := "Given ConfigValidationPassed or ConfigValidationFailed emitted, When operator calls cf_status, Then worktree_config section includes config_valid boolean and optional validation_error string so operators can diagnose misconfiguration" }]

def workflowSliceModules : List WorkflowSliceModule := [{ slice := "restore-worktree-config-from-events", formalModule := "RestoreWorktreeConfigFromEventReplay" },{ slice := "validate-config-coherence-on-startup", formalModule := "ValidateConfigCoherenceOnStartup" },{ slice := "expose-config-validity-in-status", formalModule := "ExposeConfigValidityInCfStatus" }]
...
def workflowTransitions : List WorkflowTransition := [{ source := "restore-worktree-config-from-events", target := "validate-config-coherence-on-startup", kind := WorkflowTransitionKind.event, trigger := "WorktreeConfigDeclared", sourceControl := "", targetView := "", rationale := "", payloadContract := "{ base_branch: String, worktree_dir: PathBuf, naming_template: String }" },{ source := "validate-config-coherence-on-startup", target := "expose-config-validity-in-status", kind := WorkflowTransitionKind.outcome, trigger := "ConfigValidationPassed", sourceControl := "", targetView := "", rationale := "", payloadContract := "" },{ source := "validate-config-coherence-on-startup", target := "expose-config-validity-in-status", kind := WorkflowTransitionKind.outcome, trigger := "ConfigValidationFailed", sourceControl := "", targetView := "", rationale := "", payloadContract := "" }]
...
def workflowStepRelationships : List WorkflowStepRelationship := [{ step := "restore-worktree-config-from-events", relationship := WorkflowStepRelationshipName.entry },{ step := "validate-config-coherence-on-startup", relationship := WorkflowStepRelationshipName.main },{ step := "expose-config-validity-in-status", relationship := WorkflowStepRelationshipName.main }]
...
theorem workflowIdentityIsStable : workflowName = "Validate Provisioning Config on Load" := rfl
...
end ValidateProvisioningConfigOnLoad
