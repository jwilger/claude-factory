namespace DeclareWorktreeProvisioningConfig

-- EMC-DIGEST: workflow:name=Declare Worktree Provisioning Config;slug=declare-worktree-provisioning-config;description=Workflow covering how an operator declares and updates worktree provisioning configuration: the base branch, worktree directory, and naming template that the kernel uses when creating git worktrees for parallel development work items.;slices=set-worktree-provisioning-config|Set Worktree Provisioning Configuration|state_change|Given operator calls cf_worktree_config with base_branch, worktree_dir, and naming_template, When the config is validated, Then WorktreeConfigDeclared event emitted with all configuration fields and declared_at_ms|entry,view-worktree-config-in-status|View Worktree Config in cf_status|state_view|Given WorktreeConfigDeclared event has been emitted, When operator calls cf_status, Then response includes current worktree_config with base_branch, worktree_dir, naming_template, and declared_at_ms|main;transitions=set-worktree-provisioning-config->view-worktree-config-in-status:event:WorktreeConfigDeclared::{ base_branch: String, worktree_dir: PathBuf, naming_template: String, declared_at_ms: u64 };outcomes=;command_errors=;owned_definitions=;transition_evidences=;entry_lifecycle_required=false;entry_lifecycle_states=
-- EMC generated Lean4 business workflow model.
def workflowName := "Declare Worktree Provisioning Config"

def workflowSlug := "declare-worktree-provisioning-config"

def workflowDescription := "Workflow covering how an operator declares and updates worktree provisioning configuration: the base branch, worktree directory, and naming template that the kernel uses when creating git worktrees for parallel development work items."

structure WorkflowSlice where
  slug : String

def workflowSlices : List WorkflowSlice := [{ slug := "set-worktree-provisioning-config" },{ slug := "view-worktree-config-in-status" }]
...
def workflowSliceDetails : List WorkflowSliceDetail := [{ slug := "set-worktree-provisioning-config", name := "Set Worktree Provisioning Configuration", kind := SliceKindName.stateChange, description := "Given operator calls cf_worktree_config with base_branch, worktree_dir, and naming_template, When the config is validated, Then WorktreeConfigDeclared event emitted with all configuration fields and declared_at_ms" },{ slug := "view-worktree-config-in-status", name := "View Worktree Config in cf_status", kind := SliceKindName.stateView, description := "Given WorktreeConfigDeclared event has been emitted, When operator calls cf_status, Then response includes current worktree_config with base_branch, worktree_dir, naming_template, and declared_at_ms" }]

def workflowSliceModules : List WorkflowSliceModule := [{ slice := "set-worktree-provisioning-config", formalModule := "SetWorktreeProvisioningConfiguration" },{ slice := "view-worktree-config-in-status", formalModule := "ViewWorktreeConfigInCfStatus" }]
...
def workflowTransitions : List WorkflowTransition := [{ source := "set-worktree-provisioning-config", target := "view-worktree-config-in-status", kind := WorkflowTransitionKind.event, trigger := "WorktreeConfigDeclared", sourceControl := "", targetView := "", rationale := "", payloadContract := "{ base_branch: String, worktree_dir: PathBuf, naming_template: String, declared_at_ms: u64 }" }]
...
def workflowStepRelationships : List WorkflowStepRelationship := [{ step := "set-worktree-provisioning-config", relationship := WorkflowStepRelationshipName.entry },{ step := "view-worktree-config-in-status", relationship := WorkflowStepRelationshipName.main }]
...
theorem workflowIdentityIsStable : workflowName = "Declare Worktree Provisioning Config" := rfl
...
end DeclareWorktreeProvisioningConfig
