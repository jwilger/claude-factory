namespace SnapshotCreationTrigger

-- EMC-DIGEST: workflow:name=Snapshot Creation Trigger;slug=snapshot-creation-trigger;description=Workflow covering how a replay-cost threshold breach triggers an automatic snapshot of the event store to cap future cold-start replay time.;slices=trigger-snapshot-on-threshold|Trigger Snapshot When Replay Threshold Crossed|automation|Given replay completes and duration_ms or event_count exceeds configured thresholds, When ReplayCompleted event is processed, Then CreateSnapshot command is emitted to initiate snapshot creation|entry,create-snapshot|Write Snapshot to Storage|state_change|Given CreateSnapshot command received, When current event store state is serialised and written to snapshot storage, Then SnapshotCreated event emitted with snapshot_id, event_count, and created_at_ms|main,expose-snapshot-metadata|Expose Snapshot Metadata in cf_status|state_view|Given a snapshot exists, When operator calls cf_status, Then response includes snapshot_id, snapshot_event_count, and snapshot_created_at_ms so operators can observe snapshot freshness|main;transitions=trigger-snapshot-on-threshold->create-snapshot:command:CreateSnapshot::,create-snapshot->expose-snapshot-metadata:event:SnapshotCreated::{ snapshot_id: Uuid, event_count: u64, created_at_ms: u64 };outcomes=;command_errors=;owned_definitions=;transition_evidences=;entry_lifecycle_required=false;entry_lifecycle_states=
-- EMC generated Lean4 business workflow model.
def workflowName := "Snapshot Creation Trigger"

def workflowSlug := "snapshot-creation-trigger"

def workflowDescription := "Workflow covering how a replay-cost threshold breach triggers an automatic snapshot of the event store to cap future cold-start replay time."

structure WorkflowSlice where
  slug : String

def workflowSlices : List WorkflowSlice := [{ slug := "trigger-snapshot-on-threshold" },{ slug := "create-snapshot" },{ slug := "expose-snapshot-metadata" }]
...
def workflowSliceDetails : List WorkflowSliceDetail := [{ slug := "trigger-snapshot-on-threshold", name := "Trigger Snapshot When Replay Threshold Crossed", kind := SliceKindName.automation, description := "Given replay completes and duration_ms or event_count exceeds configured thresholds, When ReplayCompleted event is processed, Then CreateSnapshot command is emitted to initiate snapshot creation" },{ slug := "create-snapshot", name := "Write Snapshot to Storage", kind := SliceKindName.stateChange, description := "Given CreateSnapshot command received, When current event store state is serialised and written to snapshot storage, Then SnapshotCreated event emitted with snapshot_id, event_count, and created_at_ms" },{ slug := "expose-snapshot-metadata", name := "Expose Snapshot Metadata in cf_status", kind := SliceKindName.stateView, description := "Given a snapshot exists, When operator calls cf_status, Then response includes snapshot_id, snapshot_event_count, and snapshot_created_at_ms so operators can observe snapshot freshness" }]

def workflowSliceModules : List WorkflowSliceModule := [{ slice := "trigger-snapshot-on-threshold", formalModule := "TriggerSnapshotWhenReplayThresholdCrossed" },{ slice := "create-snapshot", formalModule := "WriteSnapshotToStorage" },{ slice := "expose-snapshot-metadata", formalModule := "ExposeSnapshotMetadataInCfStatus" }]
...
def workflowTransitions : List WorkflowTransition := [{ source := "trigger-snapshot-on-threshold", target := "create-snapshot", kind := WorkflowTransitionKind.command, trigger := "CreateSnapshot", sourceControl := "", targetView := "", rationale := "", payloadContract := "" },{ source := "create-snapshot", target := "expose-snapshot-metadata", kind := WorkflowTransitionKind.event, trigger := "SnapshotCreated", sourceControl := "", targetView := "", rationale := "", payloadContract := "{ snapshot_id: Uuid, event_count: u64, created_at_ms: u64 }" }]
...
def workflowStepRelationships : List WorkflowStepRelationship := [{ step := "trigger-snapshot-on-threshold", relationship := WorkflowStepRelationshipName.entry },{ step := "create-snapshot", relationship := WorkflowStepRelationshipName.main },{ step := "expose-snapshot-metadata", relationship := WorkflowStepRelationshipName.main }]
...
theorem workflowIdentityIsStable : workflowName = "Snapshot Creation Trigger" := rfl
