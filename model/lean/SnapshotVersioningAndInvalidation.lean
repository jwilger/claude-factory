namespace SnapshotVersioningAndInvalidation

-- EMC-DIGEST: workflow:name=Snapshot Versioning and Invalidation;slug=snapshot-versioning-invalidation;description=Workflow covering snapshot format version checks and invalidation: when a snapshot's schema version does not match the current software version, the snapshot is invalidated and the loader falls back to full event replay.;slices=check-snapshot-schema-version|Check Snapshot Schema Version on Load|state_view|Given snapshot found in storage, When loader reads snapshot header, Then compare snapshot schema_version against current software schema_version and return Compatible or Incompatible outcome|entry,invalidate-incompatible-snapshot|Invalidate Incompatible Snapshot|state_change|Given snapshot schema_version is incompatible with current software, When loader rejects the snapshot, Then SnapshotInvalidated event emitted with snapshot_id, detected_version, expected_version, and reason=schema_mismatch|main,fallback-full-replay-after-invalidation|Fall Back to Full Replay After Invalidation|automation|Given SnapshotInvalidated event emitted, When snapshot cannot be used, Then full event replay from origin is triggered as if no snapshot were available; structured warning logged with snapshot_id and reason|main;transitions=check-snapshot-schema-version->invalidate-incompatible-snapshot:outcome:Incompatible::,invalidate-incompatible-snapshot->fallback-full-replay-after-invalidation:event:SnapshotInvalidated::{ snapshot_id: Uuid, detected_version: u32, expected_version: u32, reason: String };outcomes=;command_errors=;owned_definitions=;transition_evidences=;entry_lifecycle_required=false;entry_lifecycle_states=
-- EMC generated Lean4 business workflow model.
def workflowName := "Snapshot Versioning and Invalidation"

def workflowSlug := "snapshot-versioning-invalidation"

def workflowDescription := "Workflow covering snapshot format version checks and invalidation: when a snapshot's schema version does not match the current software version, the snapshot is invalidated and the loader falls back to full event replay."

structure WorkflowSlice where
  slug : String

def workflowSlices : List WorkflowSlice := [{ slug := "check-snapshot-schema-version" },{ slug := "invalidate-incompatible-snapshot" },{ slug := "fallback-full-replay-after-invalidation" }]
...
def workflowSliceDetails : List WorkflowSliceDetail := [{ slug := "check-snapshot-schema-version", name := "Check Snapshot Schema Version on Load", kind := SliceKindName.stateView, description := "Given snapshot found in storage, When loader reads snapshot header, Then compare snapshot schema_version against current software schema_version and return Compatible or Incompatible outcome" },{ slug := "invalidate-incompatible-snapshot", name := "Invalidate Incompatible Snapshot", kind := SliceKindName.stateChange, description := "Given snapshot schema_version is incompatible with current software, When loader rejects the snapshot, Then SnapshotInvalidated event emitted with snapshot_id, detected_version, expected_version, and reason=schema_mismatch" },{ slug := "fallback-full-replay-after-invalidation", name := "Fall Back to Full Replay After Invalidation", kind := SliceKindName.automation, description := "Given SnapshotInvalidated event emitted, When snapshot cannot be used, Then full event replay from origin is triggered as if no snapshot were available; structured warning logged with snapshot_id and reason" }]

def workflowSliceModules : List WorkflowSliceModule := [{ slice := "check-snapshot-schema-version", formalModule := "CheckSnapshotSchemaVersionOnLoad" },{ slice := "invalidate-incompatible-snapshot", formalModule := "InvalidateIncompatibleSnapshot" },{ slice := "fallback-full-replay-after-invalidation", formalModule := "FallBackToFullReplayAfterInvalidation" }]
...
def workflowTransitions : List WorkflowTransition := [{ source := "check-snapshot-schema-version", target := "invalidate-incompatible-snapshot", kind := WorkflowTransitionKind.outcome, trigger := "Incompatible", sourceControl := "", targetView := "", rationale := "", payloadContract := "" },{ source := "invalidate-incompatible-snapshot", target := "fallback-full-replay-after-invalidation", kind := WorkflowTransitionKind.event, trigger := "SnapshotInvalidated", sourceControl := "", targetView := "", rationale := "", payloadContract := "{ snapshot_id: Uuid, detected_version: u32, expected_version: u32, reason: String }" }]
...
def workflowStepRelationships : List WorkflowStepRelationship := [{ step := "check-snapshot-schema-version", relationship := WorkflowStepRelationshipName.entry },{ step := "invalidate-incompatible-snapshot", relationship := WorkflowStepRelationshipName.main },{ step := "fallback-full-replay-after-invalidation", relationship := WorkflowStepRelationshipName.main }]
...
theorem workflowIdentityIsStable : workflowName = "Snapshot Versioning and Invalidation" := rfl
...
end SnapshotVersioningAndInvalidation
