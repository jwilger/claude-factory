namespace RecordWhatWasProvisioned

-- EMC-DIGEST: workflow:name=Record What Was Provisioned;slug=record-what-was-provisioned;description=Workflow covering the durable audit record of a completed worktree provisioning: after the worktree is created and the optional setup hook completes, a WorktreeProvisioningComplete event captures the full provenance so operators can inspect the worktree inventory and cleanups are traceable.;slices=emit-provisioning-complete-record|Emit Provisioning Complete Record|state_change|Given WorktreeProvisioned and optionally SetupHookCompleted events have been recorded for a work item, When provisioning finishes, Then WorktreeProvisioningComplete event emitted with work_item_id, worktree_path, branch_name, hook_ran boolean, hook_exit_code if applicable, and completed_at_ms|entry,view-active-worktree-inventory|View Active Worktree Inventory|state_view|Given WorktreeProvisioningComplete events in the event store, When operator calls cf_status, Then response includes worktrees array with each entry showing work_item_id, worktree_path, branch_name, and completed_at_ms for audit and cleanup purposes|main;transitions=emit-provisioning-complete-record->view-active-worktree-inventory:event:WorktreeProvisioningComplete::{ work_item_id: Uuid, worktree_path: PathBuf, branch_name: String, hook_ran: bool, hook_exit_code: Option<i32>, completed_at_ms: u64 };outcomes=;command_errors=;owned_definitions=;transition_evidences=;entry_lifecycle_required=false;entry_lifecycle_states=
-- EMC generated Lean4 business workflow model.
def workflowName := "Record What Was Provisioned"

def workflowSlug := "record-what-was-provisioned"

def workflowDescription := "Workflow covering the durable audit record of a completed worktree provisioning: after the worktree is created and the optional setup hook completes, a WorktreeProvisioningComplete event captures the full provenance so operators can inspect the worktree inventory and cleanups are traceable."

structure WorkflowSlice where
  slug : String

def workflowSlices : List WorkflowSlice := [{ slug := "emit-provisioning-complete-record" },{ slug := "view-active-worktree-inventory" }]
...
def workflowSliceDetails : List WorkflowSliceDetail := [{ slug := "emit-provisioning-complete-record", name := "Emit Provisioning Complete Record", kind := SliceKindName.stateChange, description := "Given WorktreeProvisioned and optionally SetupHookCompleted events have been recorded for a work item, When provisioning finishes, Then WorktreeProvisioningComplete event emitted with work_item_id, worktree_path, branch_name, hook_ran boolean, hook_exit_code if applicable, and completed_at_ms" },{ slug := "view-active-worktree-inventory", name := "View Active Worktree Inventory", kind := SliceKindName.stateView, description := "Given WorktreeProvisioningComplete events in the event store, When operator calls cf_status, Then response includes worktrees array with each entry showing work_item_id, worktree_path, branch_name, and completed_at_ms for audit and cleanup purposes" }]

def workflowSliceModules : List WorkflowSliceModule := [{ slice := "emit-provisioning-complete-record", formalModule := "EmitProvisioningCompleteRecord" },{ slice := "view-active-worktree-inventory", formalModule := "ViewActiveWorktreeInventory" }]
...
def workflowTransitions : List WorkflowTransition := [{ source := "emit-provisioning-complete-record", target := "view-active-worktree-inventory", kind := WorkflowTransitionKind.event, trigger := "WorktreeProvisioningComplete", sourceControl := "", targetView := "", rationale := "", payloadContract := "{ work_item_id: Uuid, worktree_path: PathBuf, branch_name: String, hook_ran: bool, hook_exit_code: Option<i32>, completed_at_ms: u64 }" }]
...
def workflowStepRelationships : List WorkflowStepRelationship := [{ step := "emit-provisioning-complete-record", relationship := WorkflowStepRelationshipName.entry },{ step := "view-active-worktree-inventory", relationship := WorkflowStepRelationshipName.main }]
...
theorem workflowIdentityIsStable : workflowName = "Record What Was Provisioned" := rfl
