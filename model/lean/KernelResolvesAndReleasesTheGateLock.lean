namespace KernelResolvesAndReleasesTheGateLock

-- EMC-DIGEST: workflow:name=Kernel Resolves and Releases the Gate Lock;slug=kernel-resolves-releases-lock;description=Workflow covering the full gate lock release lifecycle: normal release after a verdict is recorded, timeout-based force-release when no verdict arrives within the configured window, and the read model update that makes lock state visible in cf_status.;slices=release-lock-after-verdict|Release Lock After Gate Verdict Recorded|automation|Given GateApproved or GateVetoed event recorded for a work item with an active gate lock, When verdict processing completes, Then GateLockReleased event emitted with work_item_id, gate_kind, verdict, held_duration_ms, and released_at_ms|entry,force-release-lock-on-timeout|Force-Release Lock on Timeout|automation|Given GateLockAcquired and no GateApproved or GateVetoed within gate_timeout_ms, When timeout expires, Then GateLockTimedOut event emitted with work_item_id and held_duration_ms; lock is force-released and work item returned to ready for a fresh gate attempt|main,view-gate-lock-state-in-status|View Gate Lock State in cf_status|state_view|Given GateLockAcquired or GateLockReleased events, When cf_status is called, Then response includes active_gate_lock field: null when no lock is held, or { work_item_id, gate_kind, acquired_at_ms } when a gate is open|main;transitions=release-lock-after-verdict->view-gate-lock-state-in-status:event:GateLockReleased::{ work_item_id: Uuid, gate_kind: String, verdict: String, held_duration_ms: u64, released_at_ms: u64 },force-release-lock-on-timeout->view-gate-lock-state-in-status:event:GateLockTimedOut::{ work_item_id: Uuid, gate_kind: String, held_duration_ms: u64 };outcomes=;command_errors=;owned_definitions=;transition_evidences=;entry_lifecycle_required=false;entry_lifecycle_states=
-- EMC generated Lean4 business workflow model.
def workflowName := "Kernel Resolves and Releases the Gate Lock"

def workflowSlug := "kernel-resolves-releases-lock"

def workflowDescription := "Workflow covering the full gate lock release lifecycle: normal release after a verdict is recorded, timeout-based force-release when no verdict arrives within the configured window, and the read model update that makes lock state visible in cf_status."

structure WorkflowSlice where
  slug : String

def workflowSlices : List WorkflowSlice := [{ slug := "release-lock-after-verdict" },{ slug := "force-release-lock-on-timeout" },{ slug := "view-gate-lock-state-in-status" }]
...
def workflowSliceDetails : List WorkflowSliceDetail := [{ slug := "release-lock-after-verdict", name := "Release Lock After Gate Verdict Recorded", kind := SliceKindName.automation, description := "Given GateApproved or GateVetoed event recorded for a work item with an active gate lock, When verdict processing completes, Then GateLockReleased event emitted with work_item_id, gate_kind, verdict, held_duration_ms, and released_at_ms" },{ slug := "force-release-lock-on-timeout", name := "Force-Release Lock on Timeout", kind := SliceKindName.automation, description := "Given GateLockAcquired and no GateApproved or GateVetoed within gate_timeout_ms, When timeout expires, Then GateLockTimedOut event emitted with work_item_id and held_duration_ms; lock is force-released and work item returned to ready for a fresh gate attempt" },{ slug := "view-gate-lock-state-in-status", name := "View Gate Lock State in cf_status", kind := SliceKindName.stateView, description := "Given GateLockAcquired or GateLockReleased events, When cf_status is called, Then response includes active_gate_lock field: null when no lock is held, or { work_item_id, gate_kind, acquired_at_ms } when a gate is open" }]

def workflowSliceModules : List WorkflowSliceModule := [{ slice := "release-lock-after-verdict", formalModule := "ReleaseLockAfterGateVerdictRecorded" },{ slice := "force-release-lock-on-timeout", formalModule := "ForceReleaseLockOnTimeout" },{ slice := "view-gate-lock-state-in-status", formalModule := "ViewGateLockStateInCfStatus" }]
...
def workflowTransitions : List WorkflowTransition := [{ source := "release-lock-after-verdict", target := "view-gate-lock-state-in-status", kind := WorkflowTransitionKind.event, trigger := "GateLockReleased", sourceControl := "", targetView := "", rationale := "", payloadContract := "{ work_item_id: Uuid, gate_kind: String, verdict: String, held_duration_ms: u64, released_at_ms: u64 }" },{ source := "force-release-lock-on-timeout", target := "view-gate-lock-state-in-status", kind := WorkflowTransitionKind.event, trigger := "GateLockTimedOut", sourceControl := "", targetView := "", rationale := "", payloadContract := "{ work_item_id: Uuid, gate_kind: String, held_duration_ms: u64 }" }]
...
def workflowStepRelationships : List WorkflowStepRelationship := [{ step := "release-lock-after-verdict", relationship := WorkflowStepRelationshipName.entry },{ step := "force-release-lock-on-timeout", relationship := WorkflowStepRelationshipName.main },{ step := "view-gate-lock-state-in-status", relationship := WorkflowStepRelationshipName.main }]
...
theorem workflowIdentityIsStable : workflowName = "Kernel Resolves and Releases the Gate Lock" := rfl
...
end KernelResolvesAndReleasesTheGateLock
