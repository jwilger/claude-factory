namespace OperatorRejectsAGate
...
def workflowName := "Operator Rejects a Gate"

def workflowSlug := "operator-rejects-gate"

def workflowDescription := "Workflow covering the gate veto path: a reviewer agent or human operator returns a Vetoed verdict via cf_gate, the kernel emits GateVetoed with the reason, and the work item is returned for rework at the appropriate earlier TDD phase."

structure WorkflowSlice where
  slug : String

def workflowSlices : List WorkflowSlice := [{ slug := "reviewer-submits-veto" },{ slug := "return-work-item-for-rework" },{ slug := "expose-rework-item-in-backlog" }]
...
def workflowSliceDetails : List WorkflowSliceDetail := [{ slug := "reviewer-submits-veto", name := "Reviewer Submits Veto via cf_gate", kind := SliceKindName.stateChange, description := "Given gate_review step completed and reviewer returns Vetoed verdict, When conductor calls cf_gate with verdict=vetoed, reviewer_id, and reason, Then GateVetoed event emitted with work_item_id, gate_kind, reviewer_id, reason, and vetoed_at_ms" },{ slug := "return-work-item-for-rework", name := "Return Work Item for Rework After Veto", kind := SliceKindName.automation, description := "Given GateVetoed event emitted, When veto recorded, Then WorkItemReturnedForRework event emitted with target_phase (WriteTest for test-review veto, Implement for implementation-review veto); work item transitions back to ready in that phase with veto reason attached" },{ slug := "expose-rework-item-in-backlog", name := "Expose Rework Item in cf_status Backlog", kind := SliceKindName.stateView, description := "Given WorkItemReturnedForRework, When cf_status or cf_backlog called, Then work item appears as ready in target_phase with veto_reason field populated so the rework agent sees why the previous attempt was rejected" }]

def workflowSliceModules : List WorkflowSliceModule := [{ slice := "reviewer-submits-veto", formalModule := "ReviewerSubmitsVetoViaCfGate" },{ slice := "return-work-item-for-rework", formalModule := "ReturnWorkItemForReworkAfterVeto" },{ slice := "expose-rework-item-in-backlog", formalModule := "ExposeReworkItemInCfStatusBacklog" }]
...
def workflowTransitions : List WorkflowTransition := [{ source := "reviewer-submits-veto", target := "return-work-item-for-rework", kind := WorkflowTransitionKind.event, trigger := "GateVetoed", sourceControl := "", targetView := "", rationale := "", payloadContract := "{ work_item_id: Uuid, gate_kind: String, reviewer_id: String, reason: String, vetoed_at_ms: u64 }" },{ source := "return-work-item-for-rework", target := "expose-rework-item-in-backlog", kind := WorkflowTransitionKind.event, trigger := "WorkItemReturnedForRework", sourceControl := "", targetView := "", rationale := "", payloadContract := "{ work_item_id: Uuid, target_phase: String, veto_reason: String }" }]
...
def workflowStepRelationships : List WorkflowStepRelationship := [{ step := "reviewer-submits-veto", relationship := WorkflowStepRelationshipName.entry },{ step := "return-work-item-for-rework", relationship := WorkflowStepRelationshipName.main },{ step := "expose-rework-item-in-backlog", relationship := WorkflowStepRelationshipName.main }]
...
theorem workflowIdentityIsStable : workflowName = "Operator Rejects a Gate" := rfl
