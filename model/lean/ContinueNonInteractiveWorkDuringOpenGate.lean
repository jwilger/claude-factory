namespace ContinueNonInteractiveWorkDuringOpenGate

-- EMC-DIGEST: workflow:name=Continue Non-Interactive Work During Open Gate;slug=continue-noninteractive-work-during-open-gate;description=Workflow covering the concurrent progress guarantee: while an interactive gate lock is held, cf_next_step must still return spawn_agent and run_check steps for other work items so the conductor can make progress without waiting for the gate verdict.;slices=schedule-non-interactive-step-while-gate-open|Schedule Non-Interactive Step While Gate Is Open|state_view|Given an interactive gate lock is held by work item A, When cf_next_step is called with no phase filter, Then return a spawn_agent or run_check step for a different work item B that does not require the gate lock; gate_review and ask_human steps for other items are deferred if they would contend|entry,execute-non-interactive-step-concurrently|Execute Non-Interactive Step Concurrently|state_change|Given non-interactive step returned by cf_next_step while gate lock is held, When conductor spawns the agent or runs the check for work item B, Then NonInteractiveStepCompleted event emitted with work_item_id B and phase; gate lock for work item A is unaffected|main,expose-concurrent-progress-in-status|Expose Concurrent Progress in cf_status|state_view|Given NonInteractiveStepCompleted events accumulating for multiple work items while a gate is open, When cf_status is called, Then in_progress counts reflect all actively progressing work items including those running concurrently alongside the open gate|main;transitions=schedule-non-interactive-step-while-gate-open->execute-non-interactive-step-concurrently:event:NonInteractiveStepScheduled::{ work_item_id: Uuid, phase: String, step_type: String },execute-non-interactive-step-concurrently->expose-concurrent-progress-in-status:event:NonInteractiveStepCompleted::{ work_item_id: Uuid, phase: String, completed_at_ms: u64 };outcomes=;command_errors=;owned_definitions=;transition_evidences=;entry_lifecycle_required=false;entry_lifecycle_states=
-- EMC generated Lean4 business workflow model.
def workflowName := "Continue Non-Interactive Work During Open Gate"

def workflowSlug := "continue-noninteractive-work-during-open-gate"

def workflowDescription := "Workflow covering the concurrent progress guarantee: while an interactive gate lock is held, cf_next_step must still return spawn_agent and run_check steps for other work items so the conductor can make progress without waiting for the gate verdict."

structure WorkflowSlice where
  slug : String

def workflowSlices : List WorkflowSlice := [{ slug := "schedule-non-interactive-step-while-gate-open" },{ slug := "execute-non-interactive-step-concurrently" },{ slug := "expose-concurrent-progress-in-status" }]
...
def workflowSliceDetails : List WorkflowSliceDetail := [{ slug := "schedule-non-interactive-step-while-gate-open", name := "Schedule Non-Interactive Step While Gate Is Open", kind := SliceKindName.stateView, description := "Given an interactive gate lock is held by work item A, When cf_next_step is called with no phase filter, Then return a spawn_agent or run_check step for a different work item B that does not require the gate lock; gate_review and ask_human steps for other items are deferred if they would contend" },{ slug := "execute-non-interactive-step-concurrently", name := "Execute Non-Interactive Step Concurrently", kind := SliceKindName.stateChange, description := "Given non-interactive step returned by cf_next_step while gate lock is held, When conductor spawns the agent or runs the check for work item B, Then NonInteractiveStepCompleted event emitted with work_item_id B and phase; gate lock for work item A is unaffected" },{ slug := "expose-concurrent-progress-in-status", name := "Expose Concurrent Progress in cf_status", kind := SliceKindName.stateView, description := "Given NonInteractiveStepCompleted events accumulating for multiple work items while a gate is open, When cf_status is called, Then in_progress counts reflect all actively progressing work items including those running concurrently alongside the open gate" }]

def workflowSliceModules : List WorkflowSliceModule := [{ slice := "schedule-non-interactive-step-while-gate-open", formalModule := "ScheduleNonInteractiveStepWhileGateIsOpen" },{ slice := "execute-non-interactive-step-concurrently", formalModule := "ExecuteNonInteractiveStepConcurrently" },{ slice := "expose-concurrent-progress-in-status", formalModule := "ExposeConcurrentProgressInCfStatus" }]
...
def workflowTransitions : List WorkflowTransition := [{ source := "schedule-non-interactive-step-while-gate-open", target := "execute-non-interactive-step-concurrently", kind := WorkflowTransitionKind.event, trigger := "NonInteractiveStepScheduled", sourceControl := "", targetView := "", rationale := "", payloadContract := "{ work_item_id: Uuid, phase: String, step_type: String }" },{ source := "execute-non-interactive-step-concurrently", target := "expose-concurrent-progress-in-status", kind := WorkflowTransitionKind.event, trigger := "NonInteractiveStepCompleted", sourceControl := "", targetView := "", rationale := "", payloadContract := "{ work_item_id: Uuid, phase: String, completed_at_ms: u64 }" }]
...
def workflowStepRelationships : List WorkflowStepRelationship := [{ step := "schedule-non-interactive-step-while-gate-open", relationship := WorkflowStepRelationshipName.entry },{ step := "execute-non-interactive-step-concurrently", relationship := WorkflowStepRelationshipName.main },{ step := "expose-concurrent-progress-in-status", relationship := WorkflowStepRelationshipName.main }]
...
theorem workflowIdentityIsStable : workflowName = "Continue Non-Interactive Work During Open Gate" := rfl
...
end ContinueNonInteractiveWorkDuringOpenGate
