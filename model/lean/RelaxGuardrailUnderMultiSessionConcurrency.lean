namespace RelaxGuardrailUnderMultiSessionConcurrency

-- EMC-DIGEST: workflow:name=Relax Guardrail Under Multi-Session Concurrency;slug=relax-guardrail-under-multi-session;description=Workflow covering guardrail adaptation for concurrent multi-session scenarios: each session holds a lease keyed to its CLAUDE_CODE_SESSION_ID and worktree_path; the guardrail permits edits within the session's own leased worktree and blocks cross-session writes to other worktrees.;slices=verify-session-lease-matches-worktree|Verify Session Lease Matches Target Worktree|state_view|Given multiple concurrent sessions each holding a lease for a different worktree_path, When a session's guardrail hook fires for an edit to a file under a given path, Then check if the session's CLAUDE_CODE_SESSION_ID matches the lease holder for that worktree_path; return Permitted or Blocked outcome|entry,permit-edit-for-leased-session|Permit Edit for Session with Valid Worktree Lease|automation|Given Permitted outcome from lease check, When guardrail hook approves the edit, Then GuardrailCheckPassed event recorded with session_id, worktree_path, and file_path; edit proceeds without interruption|main,block-cross-session-worktree-edit|Block Cross-Session Worktree Edit Attempt|state_change|Given Blocked outcome from lease check (session attempts to edit a worktree it does not own), When guardrail hook rejects the edit, Then GuardrailViolationAttempted event recorded with session_id, target_path, and lease_holder_session_id; edit is denied with a clear error message|main;transitions=verify-session-lease-matches-worktree->permit-edit-for-leased-session:outcome:Permitted::,verify-session-lease-matches-worktree->block-cross-session-worktree-edit:outcome:Blocked::;outcomes=;command_errors=;owned_definitions=;transition_evidences=;entry_lifecycle_required=false;entry_lifecycle_states=
-- EMC generated Lean4 business workflow model.
def workflowName := "Relax Guardrail Under Multi-Session Concurrency"

def workflowSlug := "relax-guardrail-under-multi-session"

def workflowDescription := "Workflow covering guardrail adaptation for concurrent multi-session scenarios: each session holds a lease keyed to its CLAUDE_CODE_SESSION_ID and worktree_path; the guardrail permits edits within the session's own leased worktree and blocks cross-session writes to other worktrees."

structure WorkflowSlice where
  slug : String

def workflowSlices : List WorkflowSlice := [{ slug := "verify-session-lease-matches-worktree" },{ slug := "permit-edit-for-leased-session" },{ slug := "block-cross-session-worktree-edit" }]
...
def workflowSliceDetails : List WorkflowSliceDetail := [{ slug := "verify-session-lease-matches-worktree", name := "Verify Session Lease Matches Target Worktree", kind := SliceKindName.stateView, description := "Given multiple concurrent sessions each holding a lease for a different worktree_path, When a session's guardrail hook fires for an edit to a file under a given path, Then check if the session's CLAUDE_CODE_SESSION_ID matches the lease holder for that worktree_path; return Permitted or Blocked outcome" },{ slug := "permit-edit-for-leased-session", name := "Permit Edit for Session with Valid Worktree Lease", kind := SliceKindName.automation, description := "Given Permitted outcome from lease check, When guardrail hook approves the edit, Then GuardrailCheckPassed event recorded with session_id, worktree_path, and file_path; edit proceeds without interruption" },{ slug := "block-cross-session-worktree-edit", name := "Block Cross-Session Worktree Edit Attempt", kind := SliceKindName.stateChange, description := "Given Blocked outcome from lease check (session attempts to edit a worktree it does not own), When guardrail hook rejects the edit, Then GuardrailViolationAttempted event recorded with session_id, target_path, and lease_holder_session_id; edit is denied with a clear error message" }]

def workflowSliceModules : List WorkflowSliceModule := [{ slice := "verify-session-lease-matches-worktree", formalModule := "VerifySessionLeaseMatchesTargetWorktree" },{ slice := "permit-edit-for-leased-session", formalModule := "PermitEditForSessionWithValidWorktreeLease" },{ slice := "block-cross-session-worktree-edit", formalModule := "BlockCrossSessionWorktreeEditAttempt" }]
...
def workflowTransitions : List WorkflowTransition := [{ source := "verify-session-lease-matches-worktree", target := "permit-edit-for-leased-session", kind := WorkflowTransitionKind.outcome, trigger := "Permitted", sourceControl := "", targetView := "", rationale := "", payloadContract := "" },{ source := "verify-session-lease-matches-worktree", target := "block-cross-session-worktree-edit", kind := WorkflowTransitionKind.outcome, trigger := "Blocked", sourceControl := "", targetView := "", rationale := "", payloadContract := "" }]
...
def workflowStepRelationships : List WorkflowStepRelationship := [{ step := "verify-session-lease-matches-worktree", relationship := WorkflowStepRelationshipName.entry },{ step := "permit-edit-for-leased-session", relationship := WorkflowStepRelationshipName.main },{ step := "block-cross-session-worktree-edit", relationship := WorkflowStepRelationshipName.main }]
...
theorem workflowIdentityIsStable : workflowName = "Relax Guardrail Under Multi-Session Concurrency" := rfl
...
end RelaxGuardrailUnderMultiSessionConcurrency
