namespace DetectNoSnapshotOnStartup

-- EMC-DIGEST: slice:name=Detect No Snapshot on Startup;slug=detect-no-snapshot-on-startup;kind=state_view;description=Given server starts cold, When loader checks snapshot storage and finds none, Then NoSnapshotAvailable outcome returned so the loader proceeds to full event replay
-- EMC generated Lean4 business slice model.
def sliceName := "Detect No Snapshot on Startup"

def sliceSlug := "detect-no-snapshot-on-startup"
...
def sliceDescription := "Given server starts cold, When loader checks snapshot storage and finds none, Then NoSnapshotAvailable outcome returned so the loader proceeds to full event replay"
...
theorem sliceIdentityIsStable : sliceName = "Detect No Snapshot on Startup" := rfl
