namespace RemoveGitWorktree

-- EMC-DIGEST: slice:name=Remove Git Worktree;slug=remove-git-worktree;kind=state_change;description=Given TeardownHookCompleted (or hook absent), When git worktree remove executes for the worktree_path, Then WorktreeTornDown event emitted with work_item_id, worktree_path, branch_name, and torn_down_at_ms; inventory entry removed from cf_status
-- EMC generated Lean4 business slice model.
def sliceName := "Remove Git Worktree"

def sliceSlug := "remove-git-worktree"
...
def sliceDescription := "Given TeardownHookCompleted (or hook absent), When git worktree remove executes for the worktree_path, Then WorktreeTornDown event emitted with work_item_id, worktree_path, branch_name, and torn_down_at_ms; inventory entry removed from cf_status"
...
theorem sliceIdentityIsStable : sliceName = "Remove Git Worktree" := rfl
