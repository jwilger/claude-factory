namespace ReleaseLockAfterGateVerdictRecorded

-- EMC-DIGEST: slice:name=Release Lock After Gate Verdict Recorded;slug=release-lock-after-verdict;kind=automation;description=Given GateApproved or GateVetoed event recorded for a work item with an active gate lock, When verdict processing completes, Then GateLockReleased event emitted with work_item_id, gate_kind, verdict, held_duration_ms, and released_at_ms
-- EMC generated Lean4 business slice model.
def sliceName := "Release Lock After Gate Verdict Recorded"

def sliceSlug := "release-lock-after-verdict"
...
def sliceDescription := "Given GateApproved or GateVetoed event recorded for a work item with an active gate lock, When verdict processing completes, Then GateLockReleased event emitted with work_item_id, gate_kind, verdict, held_duration_ms, and released_at_ms"
...
theorem sliceIdentityIsStable : sliceName = "Release Lock After Gate Verdict Recorded" := rfl
...
end ReleaseLockAfterGateVerdictRecorded
