namespace DetectAvailableSnapshotOnColdStart

-- EMC-DIGEST: slice:name=Detect Available Snapshot on Cold Start;slug=detect-available-snapshot;kind=state_view;description=Given server starts cold, When loader initialises, Then check snapshot storage and return snapshot metadata (snapshot_id, event_count, created_at_ms) if a snapshot exists, or None if no snapshot is available
-- EMC generated Lean4 business slice model.
def sliceName := "Detect Available Snapshot on Cold Start"

def sliceSlug := "detect-available-snapshot"
...
def sliceDescription := "Given server starts cold, When loader initialises, Then check snapshot storage and return snapshot metadata (snapshot_id, event_count, created_at_ms) if a snapshot exists, or None if no snapshot is available"
...
theorem sliceIdentityIsStable : sliceName = "Detect Available Snapshot on Cold Start" := rfl
...
end DetectAvailableSnapshotOnColdStart
