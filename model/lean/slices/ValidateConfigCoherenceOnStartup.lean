namespace ValidateConfigCoherenceOnStartup

-- EMC-DIGEST: slice:name=Validate Config Coherence on Startup;slug=validate-config-coherence-on-startup;kind=automation;description=Given worktree_config restored from replay, When server completes startup, Then check that base_branch exists in the git repo and worktree_dir path is accessible; emit ConfigValidationPassed or ConfigValidationFailed with reason
-- EMC generated Lean4 business slice model.
def sliceName := "Validate Config Coherence on Startup"

def sliceSlug := "validate-config-coherence-on-startup"
...
def sliceDescription := "Given worktree_config restored from replay, When server completes startup, Then check that base_branch exists in the git repo and worktree_dir path is accessible; emit ConfigValidationPassed or ConfigValidationFailed with reason"
...
theorem sliceIdentityIsStable : sliceName = "Validate Config Coherence on Startup" := rfl
...
end ValidateConfigCoherenceOnStartup
