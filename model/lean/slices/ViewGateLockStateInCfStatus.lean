namespace ViewGateLockStateInCfStatus

-- EMC-DIGEST: slice:name=View Gate Lock State in cf_status;slug=view-gate-lock-state-in-status;kind=state_view;description=Given GateLockAcquired or GateLockReleased events, When cf_status is called, Then response includes active_gate_lock field: null when no lock is held, or { work_item_id, gate_kind, acquired_at_ms } when a gate is open
-- EMC generated Lean4 business slice model.
def sliceName := "View Gate Lock State in cf_status"

def sliceSlug := "view-gate-lock-state-in-status"
...
def sliceDescription := "Given GateLockAcquired or GateLockReleased events, When cf_status is called, Then response includes active_gate_lock field: null when no lock is held, or { work_item_id, gate_kind, acquired_at_ms } when a gate is open"
...
theorem sliceIdentityIsStable : sliceName = "View Gate Lock State in cf_status" := rfl
...
end ViewGateLockStateInCfStatus
