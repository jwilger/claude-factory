namespace ViewWorktreeConfigInCfStatus

-- EMC-DIGEST: slice:name=View Worktree Config in cf_status;slug=view-worktree-config-in-status;kind=state_view;description=Given WorktreeConfigDeclared event has been emitted, When operator calls cf_status, Then response includes current worktree_config with base_branch, worktree_dir, naming_template, and declared_at_ms
-- EMC generated Lean4 business slice model.
def sliceName := "View Worktree Config in cf_status"

def sliceSlug := "view-worktree-config-in-status"
...
def sliceDescription := "Given WorktreeConfigDeclared event has been emitted, When operator calls cf_status, Then response includes current worktree_config with base_branch, worktree_dir, naming_template, and declared_at_ms"
...
theorem sliceIdentityIsStable : sliceName = "View Worktree Config in cf_status" := rfl
...
end ViewWorktreeConfigInCfStatus
