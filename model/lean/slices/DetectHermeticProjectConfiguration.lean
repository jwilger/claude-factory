namespace DetectHermeticProjectConfiguration

-- EMC-DIGEST: slice:name=Detect Hermetic Project Configuration;slug=detect-hermetic-project-flag;kind=state_view;description=Given work item claimed for development, When kernel checks worktree provisioning config, Then return Hermetic or IsolationRequired outcome based on whether worktree_isolation is disabled in project config
-- EMC generated Lean4 business slice model.
def sliceName := "Detect Hermetic Project Configuration"

def sliceSlug := "detect-hermetic-project-flag"
...
def sliceDescription := "Given work item claimed for development, When kernel checks worktree provisioning config, Then return Hermetic or IsolationRequired outcome based on whether worktree_isolation is disabled in project config"
...
theorem sliceIdentityIsStable : sliceName = "Detect Hermetic Project Configuration" := rfl
...
end DetectHermeticProjectConfiguration
