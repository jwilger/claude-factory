namespace ExposeWorktreePathInStepContext

-- EMC-DIGEST: slice:name=Expose Worktree Path in Step Context;slug=expose-worktree-path-in-step-context;kind=state_view;description=Given WorktreeProvisioned event emitted for a work item, When cf_next_step returns the next step for that work item, Then step context includes worktree_path so the spawned agent writes code in the isolated worktree directory
-- EMC generated Lean4 business slice model.
def sliceName := "Expose Worktree Path in Step Context"

def sliceSlug := "expose-worktree-path-in-step-context"
...
def sliceDescription := "Given WorktreeProvisioned event emitted for a work item, When cf_next_step returns the next step for that work item, Then step context includes worktree_path so the spawned agent writes code in the isolated worktree directory"
...
theorem sliceIdentityIsStable : sliceName = "Expose Worktree Path in Step Context" := rfl
...
end ExposeWorktreePathInStepContext
