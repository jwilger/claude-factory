namespace AssembleContextHeaderForHumanGate

-- EMC-DIGEST: slice:name=Assemble Context Header for Human Gate;slug=assemble-human-gate-context-header;kind=state_view;description=Given ask_human step is scheduled for a work item, When cf_next_step is called, Then context header assembled from work_item description, current phase, relevant prior events, and any escalation notes; returned as context_header field in the ask_human step payload
-- EMC generated Lean4 business slice model.
def sliceName := "Assemble Context Header for Human Gate"

def sliceSlug := "assemble-human-gate-context-header"
...
def sliceDescription := "Given ask_human step is scheduled for a work item, When cf_next_step is called, Then context header assembled from work_item description, current phase, relevant prior events, and any escalation notes; returned as context_header field in the ask_human step payload"
...
theorem sliceIdentityIsStable : sliceName = "Assemble Context Header for Human Gate" := rfl
...
end AssembleContextHeaderForHumanGate
