namespace DetectSetupHookScriptAfterProvisioning

-- EMC-DIGEST: slice:name=Detect Setup Hook Script After Provisioning;slug=detect-setup-hook-script;kind=automation;description=Given WorktreeProvisioned event emitted, When kernel checks for a setup hook script at the configured hook_path, Then HookPresent or HookAbsent outcome returned; absent hook is not an error
-- EMC generated Lean4 business slice model.
def sliceName := "Detect Setup Hook Script After Provisioning"

def sliceSlug := "detect-setup-hook-script"
...
def sliceDescription := "Given WorktreeProvisioned event emitted, When kernel checks for a setup hook script at the configured hook_path, Then HookPresent or HookAbsent outcome returned; absent hook is not an error"
...
theorem sliceIdentityIsStable : sliceName = "Detect Setup Hook Script After Provisioning" := rfl
...
end DetectSetupHookScriptAfterProvisioning
