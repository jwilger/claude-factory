namespace FullEventReplayFromOrigin

-- EMC-DIGEST: slice:name=Full Event Replay From Origin;slug=full-event-replay-from-origin;kind=state_change;description=Given no snapshot available and events exist, When all events are replayed from position zero, Then ReplayCompleted event emitted with total replay_duration_ms and replay_event_count; high counts trigger snapshot creation via the replay-cost-instrumentation workflow
-- EMC generated Lean4 business slice model.
def sliceName := "Full Event Replay From Origin"

def sliceSlug := "full-event-replay-from-origin"
...
def sliceDescription := "Given no snapshot available and events exist, When all events are replayed from position zero, Then ReplayCompleted event emitted with total replay_duration_ms and replay_event_count; high counts trigger snapshot creation via the replay-cost-instrumentation workflow"
...
theorem sliceIdentityIsStable : sliceName = "Full Event Replay From Origin" := rfl
