namespace ReplayDeltaEventsSinceSnapshot

-- EMC-DIGEST: slice:name=Replay Delta Events Since Snapshot;slug=replay-delta-events;kind=state_change;description=Given SnapshotLoaded event with event_count offset, When only events appended after the snapshot are replayed, Then ReplayCompleted event emitted with replay_duration_ms and replay_event_count reflecting only the delta, not full history
-- EMC generated Lean4 business slice model.
def sliceName := "Replay Delta Events Since Snapshot"

def sliceSlug := "replay-delta-events"
...
def sliceDescription := "Given SnapshotLoaded event with event_count offset, When only events appended after the snapshot are replayed, Then ReplayCompleted event emitted with replay_duration_ms and replay_event_count reflecting only the delta, not full history"
...
theorem sliceIdentityIsStable : sliceName = "Replay Delta Events Since Snapshot" := rfl
...
end ReplayDeltaEventsSinceSnapshot
