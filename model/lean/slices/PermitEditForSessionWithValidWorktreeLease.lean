namespace PermitEditForSessionWithValidWorktreeLease

-- EMC-DIGEST: slice:name=Permit Edit for Session with Valid Worktree Lease;slug=permit-edit-for-leased-session;kind=automation;description=Given Permitted outcome from lease check, When guardrail hook approves the edit, Then GuardrailCheckPassed event recorded with session_id, worktree_path, and file_path; edit proceeds without interruption
-- EMC generated Lean4 business slice model.
def sliceName := "Permit Edit for Session with Valid Worktree Lease"

def sliceSlug := "permit-edit-for-leased-session"
...
def sliceDescription := "Given Permitted outcome from lease check, When guardrail hook approves the edit, Then GuardrailCheckPassed event recorded with session_id, worktree_path, and file_path; edit proceeds without interruption"
...
theorem sliceIdentityIsStable : sliceName = "Permit Edit for Session with Valid Worktree Lease" := rfl
...
end PermitEditForSessionWithValidWorktreeLease
