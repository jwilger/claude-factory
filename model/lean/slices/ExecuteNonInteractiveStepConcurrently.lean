namespace ExecuteNonInteractiveStepConcurrently

-- EMC-DIGEST: slice:name=Execute Non-Interactive Step Concurrently;slug=execute-non-interactive-step-concurrently;kind=state_change;description=Given non-interactive step returned by cf_next_step while gate lock is held, When conductor spawns the agent or runs the check for work item B, Then NonInteractiveStepCompleted event emitted with work_item_id B and phase; gate lock for work item A is unaffected
-- EMC generated Lean4 business slice model.
def sliceName := "Execute Non-Interactive Step Concurrently"

def sliceSlug := "execute-non-interactive-step-concurrently"
...
def sliceDescription := "Given non-interactive step returned by cf_next_step while gate lock is held, When conductor spawns the agent or runs the check for work item B, Then NonInteractiveStepCompleted event emitted with work_item_id B and phase; gate lock for work item A is unaffected"
...
theorem sliceIdentityIsStable : sliceName = "Execute Non-Interactive Step Concurrently" := rfl
...
end ExecuteNonInteractiveStepConcurrently
