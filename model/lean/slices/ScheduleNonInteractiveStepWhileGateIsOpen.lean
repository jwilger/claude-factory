namespace ScheduleNonInteractiveStepWhileGateIsOpen

-- EMC-DIGEST: slice:name=Schedule Non-Interactive Step While Gate Is Open;slug=schedule-non-interactive-step-while-gate-open;kind=state_view;description=Given an interactive gate lock is held by work item A, When cf_next_step is called with no phase filter, Then return a spawn_agent or run_check step for a different work item B that does not require the gate lock; gate_review and ask_human steps for other items are deferred if they would contend
-- EMC generated Lean4 business slice model.
def sliceName := "Schedule Non-Interactive Step While Gate Is Open"

def sliceSlug := "schedule-non-interactive-step-while-gate-open"
...
def sliceDescription := "Given an interactive gate lock is held by work item A, When cf_next_step is called with no phase filter, Then return a spawn_agent or run_check step for a different work item B that does not require the gate lock; gate_review and ask_human steps for other items are deferred if they would contend"
...
theorem sliceIdentityIsStable : sliceName = "Schedule Non-Interactive Step While Gate Is Open" := rfl
...
end ScheduleNonInteractiveStepWhileGateIsOpen
