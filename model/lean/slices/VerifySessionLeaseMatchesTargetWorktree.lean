namespace VerifySessionLeaseMatchesTargetWorktree

-- EMC-DIGEST: slice:name=Verify Session Lease Matches Target Worktree;slug=verify-session-lease-matches-worktree;kind=state_view;description=Given multiple concurrent sessions each holding a lease for a different worktree_path, When a session's guardrail hook fires for an edit to a file under a given path, Then check if the session's CLAUDE_CODE_SESSION_ID matches the lease holder for that worktree_path; return Permitted or Blocked outcome
-- EMC generated Lean4 business slice model.
def sliceName := "Verify Session Lease Matches Target Worktree"

def sliceSlug := "verify-session-lease-matches-worktree"
...
def sliceDescription := "Given multiple concurrent sessions each holding a lease for a different worktree_path, When a session's guardrail hook fires for an edit to a file under a given path, Then check if the session's CLAUDE_CODE_SESSION_ID matches the lease holder for that worktree_path; return Permitted or Blocked outcome"
...
theorem sliceIdentityIsStable : sliceName = "Verify Session Lease Matches Target Worktree" := rfl
...
end VerifySessionLeaseMatchesTargetWorktree
