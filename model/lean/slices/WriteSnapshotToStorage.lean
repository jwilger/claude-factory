namespace WriteSnapshotToStorage

-- EMC-DIGEST: slice:name=Write Snapshot to Storage;slug=create-snapshot;kind=state_change;description=Given CreateSnapshot command received, When current event store state is serialised and written to snapshot storage, Then SnapshotCreated event emitted with snapshot_id, event_count, and created_at_ms
-- EMC generated Lean4 business slice model.
def sliceName := "Write Snapshot to Storage"

def sliceSlug := "create-snapshot"
...
def sliceDescription := "Given CreateSnapshot command received, When current event store state is serialised and written to snapshot storage, Then SnapshotCreated event emitted with snapshot_id, event_count, and created_at_ms"
...
theorem sliceIdentityIsStable : sliceName = "Write Snapshot to Storage" := rfl
