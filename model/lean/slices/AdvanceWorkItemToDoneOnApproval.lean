namespace AdvanceWorkItemToDoneOnApproval

-- EMC-DIGEST: slice:name=Advance Work Item to Done on Approval;slug=advance-work-item-to-done-on-approval;kind=automation;description=Given GateApproved event emitted, When the gate is the final gate for this work item's phase, Then WorkItemCompleted event emitted and work item transitions to Done state; done count for the phase increments in cf_status
-- EMC generated Lean4 business slice model.
def sliceName := "Advance Work Item to Done on Approval"

def sliceSlug := "advance-work-item-to-done-on-approval"
...
def sliceDescription := "Given GateApproved event emitted, When the gate is the final gate for this work item's phase, Then WorkItemCompleted event emitted and work item transitions to Done state; done count for the phase increments in cf_status"
...
theorem sliceIdentityIsStable : sliceName = "Advance Work Item to Done on Approval" := rfl
...
end AdvanceWorkItemToDoneOnApproval
