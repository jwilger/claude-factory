namespace InjectWorktreeContextIntoStepPrompt

-- EMC-DIGEST: slice:name=Inject Worktree Context into Step Prompt;slug=inject-worktree-context-into-step;kind=automation;description=Given WorktreeProvisioningComplete recorded for a work item, When cf_next_step generates the spawn_agent step, Then worktree_path and branch_name are appended to the step prompt so the agent knows exactly where to read and write code
-- EMC generated Lean4 business slice model.
def sliceName := "Inject Worktree Context into Step Prompt"

def sliceSlug := "inject-worktree-context-into-step"
...
def sliceDescription := "Given WorktreeProvisioningComplete recorded for a work item, When cf_next_step generates the spawn_agent step, Then worktree_path and branch_name are appended to the step prompt so the agent knows exactly where to read and write code"
...
theorem sliceIdentityIsStable : sliceName = "Inject Worktree Context into Step Prompt" := rfl
...
end InjectWorktreeContextIntoStepPrompt
