namespace ReleaseGateLockAfterVerdictRecorded

-- EMC-DIGEST: slice:name=Release Gate Lock After Verdict Recorded;slug=release-gate-lock-on-verdict;kind=automation;description=Given GateApproved or GateVetoed event emitted for the work item, When verdict is recorded and the gate closes, Then GateLockReleased event emitted with work_item_id and released_at_ms; the next gate for any work item can now be opened
-- EMC generated Lean4 business slice model.
def sliceName := "Release Gate Lock After Verdict Recorded"

def sliceSlug := "release-gate-lock-on-verdict"
...
def sliceDescription := "Given GateApproved or GateVetoed event emitted for the work item, When verdict is recorded and the gate closes, Then GateLockReleased event emitted with work_item_id and released_at_ms; the next gate for any work item can now be opened"
...
theorem sliceIdentityIsStable : sliceName = "Release Gate Lock After Verdict Recorded" := rfl
...
end ReleaseGateLockAfterVerdictRecorded
