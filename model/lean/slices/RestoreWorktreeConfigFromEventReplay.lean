namespace RestoreWorktreeConfigFromEventReplay

-- EMC-DIGEST: slice:name=Restore Worktree Config from Event Replay;slug=restore-worktree-config-from-events;kind=state_view;description=Given server replays events on startup, When WorktreeConfigDeclared event is processed, Then current worktree_config read model is populated with base_branch, worktree_dir, and naming_template
-- EMC generated Lean4 business slice model.
def sliceName := "Restore Worktree Config from Event Replay"

def sliceSlug := "restore-worktree-config-from-events"
...
def sliceDescription := "Given server replays events on startup, When WorktreeConfigDeclared event is processed, Then current worktree_config read model is populated with base_branch, worktree_dir, and naming_template"
...
theorem sliceIdentityIsStable : sliceName = "Restore Worktree Config from Event Replay" := rfl
...
end RestoreWorktreeConfigFromEventReplay
