namespace MeasureReplayCostOnStartup

-- EMC-DIGEST: slice:name=Measure Replay Cost on Startup;slug=measure-replay-cost;kind=state_change;description=Given the cfk server starts cold with no in-memory state, When the event log replay completes, Then replay_duration_ms and replay_event_count are recorded and emitted as a ReplayCompleted event.
-- EMC generated Lean4 business slice model.
def sliceName := "Measure Replay Cost on Startup"

def sliceSlug := "measure-replay-cost"
...
def sliceDescription := "Given the cfk server starts cold with no in-memory state, When the event log replay completes, Then replay_duration_ms and replay_event_count are recorded and emitted as a ReplayCompleted event."
...
theorem sliceIdentityIsStable : sliceName = "Measure Replay Cost on Startup" := rfl
...
end MeasureReplayCostOnStartup
