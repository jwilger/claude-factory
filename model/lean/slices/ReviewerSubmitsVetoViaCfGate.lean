namespace ReviewerSubmitsVetoViaCfGate

-- EMC-DIGEST: slice:name=Reviewer Submits Veto via cf_gate;slug=reviewer-submits-veto;kind=state_change;description=Given gate_review step completed and reviewer returns Vetoed verdict, When conductor calls cf_gate with verdict=vetoed, reviewer_id, and reason, Then GateVetoed event emitted with work_item_id, gate_kind, reviewer_id, reason, and vetoed_at_ms
-- EMC generated Lean4 business slice model.
def sliceName := "Reviewer Submits Veto via cf_gate"

def sliceSlug := "reviewer-submits-veto"
...
def sliceDescription := "Given gate_review step completed and reviewer returns Vetoed verdict, When conductor calls cf_gate with verdict=vetoed, reviewer_id, and reason, Then GateVetoed event emitted with work_item_id, gate_kind, reviewer_id, reason, and vetoed_at_ms"
...
theorem sliceIdentityIsStable : sliceName = "Reviewer Submits Veto via cf_gate" := rfl
...
end ReviewerSubmitsVetoViaCfGate
