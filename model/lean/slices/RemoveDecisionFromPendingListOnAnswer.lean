namespace RemoveDecisionFromPendingListOnAnswer

-- EMC-DIGEST: slice:name=Remove Decision from Pending List on Answer;slug=remove-decision-on-answer;kind=automation;description=Given HumanDecisionRecorded event emitted for a work item, When decision is submitted, Then pending_decisions read model entry for that work_item_id is removed so it no longer appears as a blocking decision
-- EMC generated Lean4 business slice model.
def sliceName := "Remove Decision from Pending List on Answer"

def sliceSlug := "remove-decision-on-answer"
...
def sliceDescription := "Given HumanDecisionRecorded event emitted for a work item, When decision is submitted, Then pending_decisions read model entry for that work_item_id is removed so it no longer appears as a blocking decision"
...
theorem sliceIdentityIsStable : sliceName = "Remove Decision from Pending List on Answer" := rfl
...
end RemoveDecisionFromPendingListOnAnswer
