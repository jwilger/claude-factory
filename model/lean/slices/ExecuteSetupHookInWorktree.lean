namespace ExecuteSetupHookInWorktree

-- EMC-DIGEST: slice:name=Execute Setup Hook in Worktree;slug=execute-setup-hook-in-worktree;kind=state_change;description=Given HookPresent outcome, When the hook script is executed inside the new worktree directory, Then SetupHookCompleted event emitted with work_item_id, exit_code, stdout_tail, and stderr_tail
-- EMC generated Lean4 business slice model.
def sliceName := "Execute Setup Hook in Worktree"

def sliceSlug := "execute-setup-hook-in-worktree"
...
def sliceDescription := "Given HookPresent outcome, When the hook script is executed inside the new worktree directory, Then SetupHookCompleted event emitted with work_item_id, exit_code, stdout_tail, and stderr_tail"
...
theorem sliceIdentityIsStable : sliceName = "Execute Setup Hook in Worktree" := rfl
...
end ExecuteSetupHookInWorktree
