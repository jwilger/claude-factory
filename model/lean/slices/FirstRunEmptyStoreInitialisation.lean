namespace FirstRunEmptyStoreInitialisation

-- EMC-DIGEST: slice:name=First-Run Empty Store Initialisation;slug=first-run-empty-store-init;kind=state_change;description=Given no snapshot and no events in the store (truly first run), When server starts, Then ServerInitialised event emitted with started_at_ms; replay is skipped and ReplayCompleted emitted with replay_event_count=0 and replay_duration_ms=0
-- EMC generated Lean4 business slice model.
def sliceName := "First-Run Empty Store Initialisation"

def sliceSlug := "first-run-empty-store-init"
...
def sliceDescription := "Given no snapshot and no events in the store (truly first run), When server starts, Then ServerInitialised event emitted with started_at_ms; replay is skipped and ReplayCompleted emitted with replay_event_count=0 and replay_duration_ms=0"
...
theorem sliceIdentityIsStable : sliceName = "First-Run Empty Store Initialisation" := rfl
...
end FirstRunEmptyStoreInitialisation
