namespace AgentExecutesTDDWorkInWorktree

-- EMC-DIGEST: slice:name=Agent Executes TDD Work in Worktree;slug=agent-executes-tdd-work-in-worktree;kind=state_change;description=Given spawn_agent step with worktree_path in prompt, When conductor runs the agent inside the worktree directory, Then AgentWorkCompleted event emitted with work_item_id, phase, and result payload ready for cf_submit
-- EMC generated Lean4 business slice model.
def sliceName := "Agent Executes TDD Work in Worktree"

def sliceSlug := "agent-executes-tdd-work-in-worktree"
...
def sliceDescription := "Given spawn_agent step with worktree_path in prompt, When conductor runs the agent inside the worktree directory, Then AgentWorkCompleted event emitted with work_item_id, phase, and result payload ready for cf_submit"
...
theorem sliceIdentityIsStable : sliceName = "Agent Executes TDD Work in Worktree" := rfl
...
end AgentExecutesTDDWorkInWorktree
