namespace SetWorktreeProvisioningConfiguration

-- EMC-DIGEST: slice:name=Set Worktree Provisioning Configuration;slug=set-worktree-provisioning-config;kind=state_change;description=Given operator calls cf_worktree_config with base_branch, worktree_dir, and naming_template, When the config is validated, Then WorktreeConfigDeclared event emitted with all configuration fields and declared_at_ms
-- EMC generated Lean4 business slice model.
def sliceName := "Set Worktree Provisioning Configuration"

def sliceSlug := "set-worktree-provisioning-config"
...
def sliceDescription := "Given operator calls cf_worktree_config with base_branch, worktree_dir, and naming_template, When the config is validated, Then WorktreeConfigDeclared event emitted with all configuration fields and declared_at_ms"
...
theorem sliceIdentityIsStable : sliceName = "Set Worktree Provisioning Configuration" := rfl
...
end SetWorktreeProvisioningConfiguration
