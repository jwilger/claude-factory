namespace ExposeHookResultInStepContext

-- EMC-DIGEST: slice:name=Expose Hook Result in Step Context;slug=expose-hook-result-in-step-context;kind=state_view;description=Given SetupHookCompleted event recorded, When cf_next_step returns the step for this work item, Then step context includes setup_hook_exit_code and setup_hook_stderr_tail so the conductor can surface hook failures to the operator
-- EMC generated Lean4 business slice model.
def sliceName := "Expose Hook Result in Step Context"

def sliceSlug := "expose-hook-result-in-step-context"
...
def sliceDescription := "Given SetupHookCompleted event recorded, When cf_next_step returns the step for this work item, Then step context includes setup_hook_exit_code and setup_hook_stderr_tail so the conductor can surface hook failures to the operator"
...
theorem sliceIdentityIsStable : sliceName = "Expose Hook Result in Step Context" := rfl
...
end ExposeHookResultInStepContext
