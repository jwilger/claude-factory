namespace ExposeConfigValidityInCfStatus

-- EMC-DIGEST: slice:name=Expose Config Validity in cf_status;slug=expose-config-validity-in-status;kind=state_view;description=Given ConfigValidationPassed or ConfigValidationFailed emitted, When operator calls cf_status, Then worktree_config section includes config_valid boolean and optional validation_error string so operators can diagnose misconfiguration
-- EMC generated Lean4 business slice model.
def sliceName := "Expose Config Validity in cf_status"

def sliceSlug := "expose-config-validity-in-status"
...
def sliceDescription := "Given ConfigValidationPassed or ConfigValidationFailed emitted, When operator calls cf_status, Then worktree_config section includes config_valid boolean and optional validation_error string so operators can diagnose misconfiguration"
...
theorem sliceIdentityIsStable : sliceName = "Expose Config Validity in cf_status" := rfl
...
end ExposeConfigValidityInCfStatus
