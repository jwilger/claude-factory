namespace ExposeHermeticSkipInCfStatus

-- EMC-DIGEST: slice:name=Expose Hermetic Skip in cf_status;slug=expose-hermetic-skip-in-status;kind=state_view;description=Given WorktreeProvisionSkipped event recorded, When cf_status called, Then worktree entry for this work item shows isolation=hermetic_skip and project_root as the effective working directory instead of a dedicated worktree_path
-- EMC generated Lean4 business slice model.
def sliceName := "Expose Hermetic Skip in cf_status"

def sliceSlug := "expose-hermetic-skip-in-status"
...
def sliceDescription := "Given WorktreeProvisionSkipped event recorded, When cf_status called, Then worktree entry for this work item shows isolation=hermetic_skip and project_root as the effective working directory instead of a dedicated worktree_path"
...
theorem sliceIdentityIsStable : sliceName = "Expose Hermetic Skip in cf_status" := rfl
...
end ExposeHermeticSkipInCfStatus
