namespace WarnWhenSnapshotThresholdCrossed

-- EMC-DIGEST: slice:name=Warn When Snapshot Threshold Crossed;slug=snapshot-threshold-watch;kind=automation;description=Given replay completes, When replay_duration_ms exceeds 250 OR replay_event_count exceeds 500, Then a structured warning is logged: "snapshot threshold crossed — consider enabling snapshotting".
-- EMC generated Lean4 business slice model.
def sliceName := "Warn When Snapshot Threshold Crossed"

def sliceSlug := "snapshot-threshold-watch"
...
def sliceDescription := "Given replay completes, When replay_duration_ms exceeds 250 OR replay_event_count exceeds 500, Then a structured warning is logged: \"snapshot threshold crossed — consider enabling snapshotting\"."
...
theorem sliceIdentityIsStable : sliceName = "Warn When Snapshot Threshold Crossed" := rfl
...
end WarnWhenSnapshotThresholdCrossed
