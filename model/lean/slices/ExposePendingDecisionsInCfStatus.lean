namespace ExposePendingDecisionsInCfStatus

-- EMC-DIGEST: slice:name=Expose Pending Decisions in cf_status;slug=expose-pending-decisions-in-status;kind=state_view;description=Given pending_decisions read model contains entries, When cf_status is called, Then response includes pending_decisions array with work_item_id, question, context_header, requested_at_ms, and waiting_ms for each outstanding gate so operators see exactly what is blocking and for how long
-- EMC generated Lean4 business slice model.
def sliceName := "Expose Pending Decisions in cf_status"

def sliceSlug := "expose-pending-decisions-in-status"
...
def sliceDescription := "Given pending_decisions read model contains entries, When cf_status is called, Then response includes pending_decisions array with work_item_id, question, context_header, requested_at_ms, and waiting_ms for each outstanding gate so operators see exactly what is blocking and for how long"
...
theorem sliceIdentityIsStable : sliceName = "Expose Pending Decisions in cf_status" := rfl
...
end ExposePendingDecisionsInCfStatus
