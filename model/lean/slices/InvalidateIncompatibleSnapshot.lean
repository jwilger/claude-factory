namespace InvalidateIncompatibleSnapshot

-- EMC-DIGEST: slice:name=Invalidate Incompatible Snapshot;slug=invalidate-incompatible-snapshot;kind=state_change;description=Given snapshot schema_version is incompatible with current software, When loader rejects the snapshot, Then SnapshotInvalidated event emitted with snapshot_id, detected_version, expected_version, and reason=schema_mismatch
-- EMC generated Lean4 business slice model.
def sliceName := "Invalidate Incompatible Snapshot"

def sliceSlug := "invalidate-incompatible-snapshot"
...
def sliceDescription := "Given snapshot schema_version is incompatible with current software, When loader rejects the snapshot, Then SnapshotInvalidated event emitted with snapshot_id, detected_version, expected_version, and reason=schema_mismatch"
...
theorem sliceIdentityIsStable : sliceName = "Invalidate Incompatible Snapshot" := rfl
...
end InvalidateIncompatibleSnapshot
