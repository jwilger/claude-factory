namespace RunTeardownHookToCleanSecrets

-- EMC-DIGEST: slice:name=Run Teardown Hook to Clean Secrets;slug=run-teardown-hook-clean-secrets;kind=state_change;description=Given TeardownWorktree command and optional teardown hook script present, When hook executes inside the worktree directory, Then TeardownHookCompleted event emitted with work_item_id, exit_code, and stderr_tail; non-zero exit is recorded but does not block worktree removal
-- EMC generated Lean4 business slice model.
def sliceName := "Run Teardown Hook to Clean Secrets"

def sliceSlug := "run-teardown-hook-clean-secrets"
...
def sliceDescription := "Given TeardownWorktree command and optional teardown hook script present, When hook executes inside the worktree directory, Then TeardownHookCompleted event emitted with work_item_id, exit_code, and stderr_tail; non-zero exit is recorded but does not block worktree removal"
...
theorem sliceIdentityIsStable : sliceName = "Run Teardown Hook to Clean Secrets" := rfl
...
end RunTeardownHookToCleanSecrets
