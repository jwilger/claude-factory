namespace TrackUnansweredHumanDecisionsInReadModel
...
def sliceName := "Track Unanswered Human Decisions in Read Model"

def sliceSlug := "track-unanswered-human-decisions"
...
def sliceDescription := "Given HumanDecisionRequested events without a corresponding HumanDecisionRecorded for the same work_item_id, When read model is built from event replay, Then pending_decisions list maintained with work_item_id, question, context_header, and requested_at_ms for each unanswered gate"
...
theorem sliceIdentityIsStable : sliceName = "Track Unanswered Human Decisions in Read Model" := rfl
...
end TrackUnansweredHumanDecisionsInReadModel
