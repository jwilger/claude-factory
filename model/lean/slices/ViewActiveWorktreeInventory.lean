namespace ViewActiveWorktreeInventory

-- EMC-DIGEST: slice:name=View Active Worktree Inventory;slug=view-active-worktree-inventory;kind=state_view;description=Given WorktreeProvisioningComplete events in the event store, When operator calls cf_status, Then response includes worktrees array with each entry showing work_item_id, worktree_path, branch_name, and completed_at_ms for audit and cleanup purposes
-- EMC generated Lean4 business slice model.
def sliceName := "View Active Worktree Inventory"

def sliceSlug := "view-active-worktree-inventory"
...
def sliceDescription := "Given WorktreeProvisioningComplete events in the event store, When operator calls cf_status, Then response includes worktrees array with each entry showing work_item_id, worktree_path, branch_name, and completed_at_ms for audit and cleanup purposes"
...
theorem sliceIdentityIsStable : sliceName = "View Active Worktree Inventory" := rfl
...
end ViewActiveWorktreeInventory
