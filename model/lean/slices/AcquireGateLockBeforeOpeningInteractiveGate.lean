namespace AcquireGateLockBeforeOpeningInteractiveGate

-- EMC-DIGEST: slice:name=Acquire Gate Lock Before Opening Interactive Gate;slug=acquire-gate-lock-before-opening;kind=state_change;description=Given gate_review or ask_human step is scheduled and no gate lock is currently held for this work item, When kernel prepares to open the gate, Then GateLockAcquired event emitted with work_item_id, gate_kind, and acquired_at_ms; concurrent gate attempts for the same item are rejected
-- EMC generated Lean4 business slice model.
def sliceName := "Acquire Gate Lock Before Opening Interactive Gate"

def sliceSlug := "acquire-gate-lock-before-opening"
...
def sliceDescription := "Given gate_review or ask_human step is scheduled and no gate lock is currently held for this work item, When kernel prepares to open the gate, Then GateLockAcquired event emitted with work_item_id, gate_kind, and acquired_at_ms; concurrent gate attempts for the same item are rejected"
...
theorem sliceIdentityIsStable : sliceName = "Acquire Gate Lock Before Opening Interactive Gate" := rfl
...
end AcquireGateLockBeforeOpeningInteractiveGate
