namespace ReEnqueueDeferredGateWhenLockReleased

-- EMC-DIGEST: slice:name=Re-enqueue Deferred Gate When Lock Released;slug=reenqueue-deferred-gate-on-lock-release;kind=automation;description=Given GateLockReleased event emitted by work item A, When kernel scans for deferred gates, Then any work item with a GateDeferred record where blocking_work_item_id matches A is re-enqueued as ready so the next cf_next_step call will schedule it
-- EMC generated Lean4 business slice model.
def sliceName := "Re-enqueue Deferred Gate When Lock Released"

def sliceSlug := "reenqueue-deferred-gate-on-lock-release"
...
def sliceDescription := "Given GateLockReleased event emitted by work item A, When kernel scans for deferred gates, Then any work item with a GateDeferred record where blocking_work_item_id matches A is re-enqueued as ready so the next cf_next_step call will schedule it"
...
theorem sliceIdentityIsStable : sliceName = "Re-enqueue Deferred Gate When Lock Released" := rfl
...
end ReEnqueueDeferredGateWhenLockReleased
