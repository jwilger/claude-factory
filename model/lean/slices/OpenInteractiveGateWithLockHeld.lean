namespace OpenInteractiveGateWithLockHeld

-- EMC-DIGEST: slice:name=Open Interactive Gate with Lock Held;slug=open-interactive-gate-with-lock-held;kind=state_change;description=Given GateLockAcquired, When reviewer agent is spawned or human question is presented, Then InteractiveGateOpened event emitted with work_item_id, gate_kind, and opened_at_ms confirming the gate is active
-- EMC generated Lean4 business slice model.
def sliceName := "Open Interactive Gate with Lock Held"

def sliceSlug := "open-interactive-gate-with-lock-held"
...
def sliceDescription := "Given GateLockAcquired, When reviewer agent is spawned or human question is presented, Then InteractiveGateOpened event emitted with work_item_id, gate_kind, and opened_at_ms confirming the gate is active"
...
theorem sliceIdentityIsStable : sliceName = "Open Interactive Gate with Lock Held" := rfl
...
end OpenInteractiveGateWithLockHeld
