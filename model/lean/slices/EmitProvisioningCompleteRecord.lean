namespace EmitProvisioningCompleteRecord

-- EMC-DIGEST: slice:name=Emit Provisioning Complete Record;slug=emit-provisioning-complete-record;kind=state_change;description=Given WorktreeProvisioned and optionally SetupHookCompleted events have been recorded for a work item, When provisioning finishes, Then WorktreeProvisioningComplete event emitted with work_item_id, worktree_path, branch_name, hook_ran boolean, hook_exit_code if applicable, and completed_at_ms
-- EMC generated Lean4 business slice model.
def sliceName := "Emit Provisioning Complete Record"

def sliceSlug := "emit-provisioning-complete-record"
...
def sliceDescription := "Given WorktreeProvisioned and optionally SetupHookCompleted events have been recorded for a work item, When provisioning finishes, Then WorktreeProvisioningComplete event emitted with work_item_id, worktree_path, branch_name, hook_ran boolean, hook_exit_code if applicable, and completed_at_ms"
...
theorem sliceIdentityIsStable : sliceName = "Emit Provisioning Complete Record" := rfl
...
end EmitProvisioningCompleteRecord
