...
def sliceDescription := "Given HumanDecisionRequested, When operator provides their decision and conductor calls cf_decide, Then HumanDecisionRecorded event emitted with work_item_id, decision, and decided_at_ms; work item advances to the appropriate next state"
...
theorem sliceIdentityIsStable : sliceName = "Record Human Decision" := rfl
