namespace FallBackToFullReplayAfterInvalidation

-- EMC-DIGEST: slice:name=Fall Back to Full Replay After Invalidation;slug=fallback-full-replay-after-invalidation;kind=automation;description=Given SnapshotInvalidated event emitted, When snapshot cannot be used, Then full event replay from origin is triggered as if no snapshot were available; structured warning logged with snapshot_id and reason
-- EMC generated Lean4 business slice model.
def sliceName := "Fall Back to Full Replay After Invalidation"

def sliceSlug := "fallback-full-replay-after-invalidation"
...
def sliceDescription := "Given SnapshotInvalidated event emitted, When snapshot cannot be used, Then full event replay from origin is triggered as if no snapshot were available; structured warning logged with snapshot_id and reason"
...
theorem sliceIdentityIsStable : sliceName = "Fall Back to Full Replay After Invalidation" := rfl
...
end FallBackToFullReplayAfterInvalidation
