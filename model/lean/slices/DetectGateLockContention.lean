namespace DetectGateLockContention

-- EMC-DIGEST: slice:name=Detect Gate Lock Contention;slug=detect-gate-lock-contention;kind=state_view;description=Given a gate step is scheduled for work item B, When kernel checks current gate lock state and finds GateLockAcquired for a different work item A, Then GateContention outcome returned with blocking_work_item_id so the kernel knows it cannot open gate B yet
-- EMC generated Lean4 business slice model.
def sliceName := "Detect Gate Lock Contention"

def sliceSlug := "detect-gate-lock-contention"
...
def sliceDescription := "Given a gate step is scheduled for work item B, When kernel checks current gate lock state and finds GateLockAcquired for a different work item A, Then GateContention outcome returned with blocking_work_item_id so the kernel knows it cannot open gate B yet"
...
theorem sliceIdentityIsStable : sliceName = "Detect Gate Lock Contention" := rfl
