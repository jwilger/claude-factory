namespace EmitWorktreeProvisionSkippedForHermeticProject

-- EMC-DIGEST: slice:name=Emit WorktreeProvisionSkipped for Hermetic Project;slug=emit-worktree-provision-skipped;kind=state_change;description=Given Hermetic outcome detected, When worktree provisioning would normally run, Then WorktreeProvisionSkipped event emitted with work_item_id and reason=hermetic_project so the TDD state machine can proceed using the shared checkout path
-- EMC generated Lean4 business slice model.
def sliceName := "Emit WorktreeProvisionSkipped for Hermetic Project"

def sliceSlug := "emit-worktree-provision-skipped"
...
def sliceDescription := "Given Hermetic outcome detected, When worktree provisioning would normally run, Then WorktreeProvisionSkipped event emitted with work_item_id and reason=hermetic_project so the TDD state machine can proceed using the shared checkout path"
...
theorem sliceIdentityIsStable : sliceName = "Emit WorktreeProvisionSkipped for Hermetic Project" := rfl
...
end EmitWorktreeProvisionSkippedForHermeticProject
