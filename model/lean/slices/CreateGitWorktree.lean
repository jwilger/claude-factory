namespace CreateGitWorktree

-- EMC-DIGEST: slice:name=Create Git Worktree;slug=create-git-worktree;kind=state_change;description=Given ProvisionWorktree command received, When git worktree add is executed with the derived branch name and worktree_path, Then WorktreeProvisioned event emitted with work_item_id, worktree_path, branch_name, and provisioned_at_ms
-- EMC generated Lean4 business slice model.
def sliceName := "Create Git Worktree"

def sliceSlug := "create-git-worktree"
...
def sliceDescription := "Given ProvisionWorktree command received, When git worktree add is executed with the derived branch name and worktree_path, Then WorktreeProvisioned event emitted with work_item_id, worktree_path, branch_name, and provisioned_at_ms"
...
theorem sliceIdentityIsStable : sliceName = "Create Git Worktree" := rfl
