namespace BlockCrossSessionWorktreeEditAttempt

-- EMC-DIGEST: slice:name=Block Cross-Session Worktree Edit Attempt;slug=block-cross-session-worktree-edit;kind=state_change;description=Given Blocked outcome from lease check (session attempts to edit a worktree it does not own), When guardrail hook rejects the edit, Then GuardrailViolationAttempted event recorded with session_id, target_path, and lease_holder_session_id; edit is denied with a clear error message
-- EMC generated Lean4 business slice model.
def sliceName := "Block Cross-Session Worktree Edit Attempt"

def sliceSlug := "block-cross-session-worktree-edit"
...
def sliceDescription := "Given Blocked outcome from lease check (session attempts to edit a worktree it does not own), When guardrail hook rejects the edit, Then GuardrailViolationAttempted event recorded with session_id, target_path, and lease_holder_session_id; edit is denied with a clear error message"
...
theorem sliceIdentityIsStable : sliceName = "Block Cross-Session Worktree Edit Attempt" := rfl
...
end BlockCrossSessionWorktreeEditAttempt
