namespace ExposeCompletedWorkItemInCfStatus

-- EMC-DIGEST: slice:name=Expose Completed Work Item in cf_status;slug=expose-completed-item-in-status;kind=state_view;description=Given WorkItemCompleted event emitted, When cf_status is called, Then phase row shows the done count incremented by 1 and in_progress count decremented by 1; completed item no longer appears in active backlog
-- EMC generated Lean4 business slice model.
def sliceName := "Expose Completed Work Item in cf_status"

def sliceSlug := "expose-completed-item-in-status"
...
def sliceDescription := "Given WorkItemCompleted event emitted, When cf_status is called, Then phase row shows the done count incremented by 1 and in_progress count decremented by 1; completed item no longer appears in active backlog"
...
theorem sliceIdentityIsStable : sliceName = "Expose Completed Work Item in cf_status" := rfl
...
end ExposeCompletedWorkItemInCfStatus
