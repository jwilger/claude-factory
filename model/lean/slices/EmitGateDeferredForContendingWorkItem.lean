namespace EmitGateDeferredForContendingWorkItem

-- EMC-DIGEST: slice:name=Emit GateDeferred for Contending Work Item;slug=emit-gate-deferred;kind=state_change;description=Given GateContention detected for work item B, When kernel cannot open the gate immediately, Then GateDeferred event emitted with work_item_id, gate_kind, blocking_work_item_id, and deferred_at_ms; work item B moves to waiting state until the lock is released
-- EMC generated Lean4 business slice model.
def sliceName := "Emit GateDeferred for Contending Work Item"

def sliceSlug := "emit-gate-deferred"
...
def sliceDescription := "Given GateContention detected for work item B, When kernel cannot open the gate immediately, Then GateDeferred event emitted with work_item_id, gate_kind, blocking_work_item_id, and deferred_at_ms; work item B moves to waiting state until the lock is released"
...
theorem sliceIdentityIsStable : sliceName = "Emit GateDeferred for Contending Work Item" := rfl
...
end EmitGateDeferredForContendingWorkItem
