namespace CheckSnapshotSchemaVersionOnLoad

-- EMC-DIGEST: slice:name=Check Snapshot Schema Version on Load;slug=check-snapshot-schema-version;kind=state_view;description=Given snapshot found in storage, When loader reads snapshot header, Then compare snapshot schema_version against current software schema_version and return Compatible or Incompatible outcome
-- EMC generated Lean4 business slice model.
def sliceName := "Check Snapshot Schema Version on Load"

def sliceSlug := "check-snapshot-schema-version"
...
def sliceDescription := "Given snapshot found in storage, When loader reads snapshot header, Then compare snapshot schema_version against current software schema_version and return Compatible or Incompatible outcome"
...
theorem sliceIdentityIsStable : sliceName = "Check Snapshot Schema Version on Load" := rfl
...
end CheckSnapshotSchemaVersionOnLoad
