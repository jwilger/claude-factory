namespace EmitProvisionWorktreeCommandOnClaim

-- EMC-DIGEST: slice:name=Emit ProvisionWorktree Command on Claim;slug=emit-provision-worktree-command;kind=automation;description=Given development work item claimed and worktree config is valid, When cf_claim succeeds, Then ProvisionWorktree command emitted with work_item_id, base_branch, and worktree_path derived from naming_template
-- EMC generated Lean4 business slice model.
def sliceName := "Emit ProvisionWorktree Command on Claim"

def sliceSlug := "emit-provision-worktree-command"
...
def sliceDescription := "Given development work item claimed and worktree config is valid, When cf_claim succeeds, Then ProvisionWorktree command emitted with work_item_id, base_branch, and worktree_path derived from naming_template"
...
theorem sliceIdentityIsStable : sliceName = "Emit ProvisionWorktree Command on Claim" := rfl
...
end EmitProvisionWorktreeCommandOnClaim
