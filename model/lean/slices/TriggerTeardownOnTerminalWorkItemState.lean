namespace TriggerTeardownOnTerminalWorkItemState

-- EMC-DIGEST: slice:name=Trigger Teardown on Terminal Work Item State;slug=trigger-teardown-on-terminal-state;kind=automation;description=Given work item transitions to Done or Abandoned state and a WorktreeProvisioningComplete record exists for that work item, When terminal event is emitted, Then TeardownWorktree command emitted with work_item_id and worktree_path from the provisioning record
-- EMC generated Lean4 business slice model.
def sliceName := "Trigger Teardown on Terminal Work Item State"

def sliceSlug := "trigger-teardown-on-terminal-state"
...
def sliceKind : SliceKindName := SliceKindName.automation

def sliceDescription := "Given work item transitions to Done or Abandoned state and a WorktreeProvisioningComplete record exists for that work item, When terminal event is emitted, Then TeardownWorktree command emitted with work_item_id and worktree_path from the provisioning record"
...
theorem sliceIdentityIsStable : sliceName = "Trigger Teardown on Terminal Work Item State" := rfl
...
end TriggerTeardownOnTerminalWorkItemState
