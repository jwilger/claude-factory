namespace PresentHumanDecisionRequestToOperator

-- EMC-DIGEST: slice:name=Present Human Decision Request to Operator;slug=present-human-decision-request;kind=state_change;description=Given context header assembled, When conductor displays the ask_human gate to the operator with the question and context header, Then HumanDecisionRequested event emitted with work_item_id, question, context_header, and requested_at_ms
-- EMC generated Lean4 business slice model.
def sliceName := "Present Human Decision Request to Operator"

def sliceSlug := "present-human-decision-request"
...
def sliceDescription := "Given context header assembled, When conductor displays the ask_human gate to the operator with the question and context header, Then HumanDecisionRequested event emitted with work_item_id, question, context_header, and requested_at_ms"
...
theorem sliceIdentityIsStable : sliceName = "Present Human Decision Request to Operator" := rfl
...
end PresentHumanDecisionRequestToOperator
