namespace ExposeReworkItemInCfStatusBacklog

-- EMC-DIGEST: slice:name=Expose Rework Item in cf_status Backlog;slug=expose-rework-item-in-backlog;kind=state_view;description=Given WorkItemReturnedForRework, When cf_status or cf_backlog called, Then work item appears as ready in target_phase with veto_reason field populated so the rework agent sees why the previous attempt was rejected
-- EMC generated Lean4 business slice model.
def sliceName := "Expose Rework Item in cf_status Backlog"

def sliceSlug := "expose-rework-item-in-backlog"
...
def sliceDescription := "Given WorkItemReturnedForRework, When cf_status or cf_backlog called, Then work item appears as ready in target_phase with veto_reason field populated so the rework agent sees why the previous attempt was rejected"
...
theorem sliceIdentityIsStable : sliceName = "Expose Rework Item in cf_status Backlog" := rfl
...
end ExposeReworkItemInCfStatusBacklog
