namespace ExposeConcurrentProgressInCfStatus

-- EMC-DIGEST: slice:name=Expose Concurrent Progress in cf_status;slug=expose-concurrent-progress-in-status;kind=state_view;description=Given NonInteractiveStepCompleted events accumulating for multiple work items while a gate is open, When cf_status is called, Then in_progress counts reflect all actively progressing work items including those running concurrently alongside the open gate
-- EMC generated Lean4 business slice model.
def sliceName := "Expose Concurrent Progress in cf_status"

def sliceSlug := "expose-concurrent-progress-in-status"
...
def sliceDescription := "Given NonInteractiveStepCompleted events accumulating for multiple work items while a gate is open, When cf_status is called, Then in_progress counts reflect all actively progressing work items including those running concurrently alongside the open gate"
...
theorem sliceIdentityIsStable : sliceName = "Expose Concurrent Progress in cf_status" := rfl
...
end ExposeConcurrentProgressInCfStatus
