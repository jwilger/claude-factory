namespace ForceReleaseLockOnTimeout

-- EMC-DIGEST: slice:name=Force-Release Lock on Timeout;slug=force-release-lock-on-timeout;kind=automation;description=Given GateLockAcquired and no GateApproved or GateVetoed within gate_timeout_ms, When timeout expires, Then GateLockTimedOut event emitted with work_item_id and held_duration_ms; lock is force-released and work item returned to ready for a fresh gate attempt
-- EMC generated Lean4 business slice model.
def sliceName := "Force-Release Lock on Timeout"

def sliceSlug := "force-release-lock-on-timeout"
...
def sliceDescription := "Given GateLockAcquired and no GateApproved or GateVetoed within gate_timeout_ms, When timeout expires, Then GateLockTimedOut event emitted with work_item_id and held_duration_ms; lock is force-released and work item returned to ready for a fresh gate attempt"
...
theorem sliceIdentityIsStable : sliceName = "Force-Release Lock on Timeout" := rfl
