namespace ExposeReplayMetricsInCfStatus

-- EMC-DIGEST: slice:name=Expose Replay Metrics in cf_status;slug=expose-replay-metrics;kind=state_view;description=Given replay has completed and metrics are recorded, When the operator calls cf_status, Then the response includes last_replay_ms and last_replay_event_count fields reflecting the most recent startup replay.
-- EMC generated Lean4 business slice model.
def sliceName := "Expose Replay Metrics in cf_status"

def sliceSlug := "expose-replay-metrics"
...
def sliceDescription := "Given replay has completed and metrics are recorded, When the operator calls cf_status, Then the response includes last_replay_ms and last_replay_event_count fields reflecting the most recent startup replay."
...
theorem sliceIdentityIsStable : sliceName = "Expose Replay Metrics in cf_status" := rfl
...
end ExposeReplayMetricsInCfStatus
