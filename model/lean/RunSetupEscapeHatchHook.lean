namespace RunSetupEscapeHatchHook

-- EMC-DIGEST: workflow:name=Run Setup Escape-Hatch Hook;slug=run-setup-escape-hatch-hook;description=Workflow covering the optional project-specific setup hook that runs after a worktree is provisioned. If a hook script exists at the configured path, it is executed in the new worktree; success or failure is recorded as an event so the conductor can report hook failures without blocking worktree use.;slices=detect-setup-hook-script|Detect Setup Hook Script After Provisioning|automation|Given WorktreeProvisioned event emitted, When kernel checks for a setup hook script at the configured hook_path, Then HookPresent or HookAbsent outcome returned; absent hook is not an error|entry,execute-setup-hook-in-worktree|Execute Setup Hook in Worktree|state_change|Given HookPresent outcome, When the hook script is executed inside the new worktree directory, Then SetupHookCompleted event emitted with work_item_id, exit_code, stdout_tail, and stderr_tail|main,expose-hook-result-in-step-context|Expose Hook Result in Step Context|state_view|Given SetupHookCompleted event recorded, When cf_next_step returns the step for this work item, Then step context includes setup_hook_exit_code and setup_hook_stderr_tail so the conductor can surface hook failures to the operator|main;transitions=detect-setup-hook-script->execute-setup-hook-in-worktree:outcome:HookPresent::,execute-setup-hook-in-worktree->expose-hook-result-in-step-context:event:SetupHookCompleted::{ work_item_id: Uuid, exit_code: i32, stdout_tail: String, stderr_tail: String };outcomes=;command_errors=;owned_definitions=;transition_evidences=;entry_lifecycle_required=false;entry_lifecycle_states=
-- EMC generated Lean4 business workflow model.
def workflowName := "Run Setup Escape-Hatch Hook"

def workflowSlug := "run-setup-escape-hatch-hook"

def workflowDescription := "Workflow covering the optional project-specific setup hook that runs after a worktree is provisioned. If a hook script exists at the configured path, it is executed in the new worktree; success or failure is recorded as an event so the conductor can report hook failures without blocking worktree use."

structure WorkflowSlice where
  slug : String

def workflowSlices : List WorkflowSlice := [{ slug := "detect-setup-hook-script" },{ slug := "execute-setup-hook-in-worktree" },{ slug := "expose-hook-result-in-step-context" }]
...
def workflowSliceDetails : List WorkflowSliceDetail := [{ slug := "detect-setup-hook-script", name := "Detect Setup Hook Script After Provisioning", kind := SliceKindName.automation, description := "Given WorktreeProvisioned event emitted, When kernel checks for a setup hook script at the configured hook_path, Then HookPresent or HookAbsent outcome returned; absent hook is not an error" },{ slug := "execute-setup-hook-in-worktree", name := "Execute Setup Hook in Worktree", kind := SliceKindName.stateChange, description := "Given HookPresent outcome, When the hook script is executed inside the new worktree directory, Then SetupHookCompleted event emitted with work_item_id, exit_code, stdout_tail, and stderr_tail" },{ slug := "expose-hook-result-in-step-context", name := "Expose Hook Result in Step Context", kind := SliceKindName.stateView, description := "Given SetupHookCompleted event recorded, When cf_next_step returns the step for this work item, Then step context includes setup_hook_exit_code and setup_hook_stderr_tail so the conductor can surface hook failures to the operator" }]

def workflowSliceModules : List WorkflowSliceModule := [{ slice := "detect-setup-hook-script", formalModule := "DetectSetupHookScriptAfterProvisioning" },{ slice := "execute-setup-hook-in-worktree", formalModule := "ExecuteSetupHookInWorktree" },{ slice := "expose-hook-result-in-step-context", formalModule := "ExposeHookResultInStepContext" }]
...
def workflowTransitions : List WorkflowTransition := [{ source := "detect-setup-hook-script", target := "execute-setup-hook-in-worktree", kind := WorkflowTransitionKind.outcome, trigger := "HookPresent", sourceControl := "", targetView := "", rationale := "", payloadContract := "" },{ source := "execute-setup-hook-in-worktree", target := "expose-hook-result-in-step-context", kind := WorkflowTransitionKind.event, trigger := "SetupHookCompleted", sourceControl := "", targetView := "", rationale := "", payloadContract := "{ work_item_id: Uuid, exit_code: i32, stdout_tail: String, stderr_tail: String }" }]
...
def workflowStepRelationships : List WorkflowStepRelationship := [{ step := "detect-setup-hook-script", relationship := WorkflowStepRelationshipName.entry },{ step := "execute-setup-hook-in-worktree", relationship := WorkflowStepRelationshipName.main },{ step := "expose-hook-result-in-step-context", relationship := WorkflowStepRelationshipName.main }]
...
theorem workflowIdentityIsStable : workflowName = "Run Setup Escape-Hatch Hook" := rfl
