namespace InspectPendingDecisions

-- EMC-DIGEST: workflow:name=Inspect Pending Decisions;slug=inspect-pending-decisions;description=Workflow covering the operator view of outstanding human decision gates: any ask_human step that has been presented but not yet answered appears in the pending_decisions read model, visible via cf_status, with question text, context header, and wait duration so the operator knows what is blocking progress.;slices=track-unanswered-human-decisions|Track Unanswered Human Decisions in Read Model|state_view|Given HumanDecisionRequested events without a corresponding HumanDecisionRecorded for the same work_item_id, When read model is built from event replay, Then pending_decisions list maintained with work_item_id, question, context_header, and requested_at_ms for each unanswered gate|entry,remove-decision-on-answer|Remove Decision from Pending List on Answer|automation|Given HumanDecisionRecorded event emitted for a work item, When decision is submitted, Then pending_decisions read model entry for that work_item_id is removed so it no longer appears as a blocking decision|main,expose-pending-decisions-in-status|Expose Pending Decisions in cf_status|state_view|Given pending_decisions read model contains entries, When cf_status is called, Then response includes pending_decisions array with work_item_id, question, context_header, requested_at_ms, and waiting_ms for each outstanding gate so operators see exactly what is blocking and for how long|main;transitions=track-unanswered-human-decisions->remove-decision-on-answer:event:HumanDecisionRequested::{ work_item_id: Uuid, question: String, context_header: String, requested_at_ms: u64 },remove-decision-on-answer->expose-pending-decisions-in-status:event:HumanDecisionRecorded::{ work_item_id: Uuid, decision: String, decided_at_ms: u64 };outcomes=;command_errors=;owned_definitions=;transition_evidences=;entry_lifecycle_required=false;entry_lifecycle_states=
-- EMC generated Lean4 business workflow model.
def workflowName := "Inspect Pending Decisions"

def workflowSlug := "inspect-pending-decisions"
...
def workflowSlices : List WorkflowSlice := [{ slug := "track-unanswered-human-decisions" },{ slug := "remove-decision-on-answer" },{ slug := "expose-pending-decisions-in-status" }]
...
def workflowSliceDetails : List WorkflowSliceDetail := [{ slug := "track-unanswered-human-decisions", name := "Track Unanswered Human Decisions in Read Model", kind := SliceKindName.stateView, description := "Given HumanDecisionRequested events without a corresponding HumanDecisionRecorded for the same work_item_id, When read model is built from event replay, Then pending_decisions list maintained with work_item_id, question, context_header, and requested_at_ms for each unanswered gate" },{ slug := "remove-decision-on-answer", name := "Remove Decision from Pending List on Answer", kind := SliceKindName.automation, description := "Given HumanDecisionRecorded event emitted for a work item, When decision is submitted, Then pending_decisions read model entry for that work_item_id is removed so it no longer appears as a blocking decision" },{ slug := "expose-pending-decisions-in-status", name := "Expose Pending Decisions in cf_status", kind := SliceKindName.stateView, description := "Given pending_decisions read model contains entries, When cf_status is called, Then response includes pending_decisions array with work_item_id, question, context_header, requested_at_ms, and waiting_ms for each outstanding gate so operators see exactly what is blocking and for how long" }]

def workflowSliceModules : List WorkflowSliceModule := [{ slice := "track-unanswered-human-decisions", formalModule := "TrackUnansweredHumanDecisionsInReadModel" },{ slice := "remove-decision-on-answer", formalModule := "RemoveDecisionFromPendingListOnAnswer" },{ slice := "expose-pending-decisions-in-status", formalModule := "ExposePendingDecisionsInCfStatus" }]
...
def workflowTransitions : List WorkflowTransition := [{ source := "track-unanswered-human-decisions", target := "remove-decision-on-answer", kind := WorkflowTransitionKind.event, trigger := "HumanDecisionRequested", sourceControl := "", targetView := "", rationale := "", payloadContract := "{ work_item_id: Uuid, question: String, context_header: String, requested_at_ms: u64 }" },{ source := "remove-decision-on-answer", target := "expose-pending-decisions-in-status", kind := WorkflowTransitionKind.event, trigger := "HumanDecisionRecorded", sourceControl := "", targetView := "", rationale := "", payloadContract := "{ work_item_id: Uuid, decision: String, decided_at_ms: u64 }" }]
...
def workflowStepRelationships : List WorkflowStepRelationship := [{ step := "track-unanswered-human-decisions", relationship := WorkflowStepRelationshipName.entry },{ step := "remove-decision-on-answer", relationship := WorkflowStepRelationshipName.main },{ step := "expose-pending-decisions-in-status", relationship := WorkflowStepRelationshipName.main }]
...
theorem workflowIdentityIsStable : workflowName = "Inspect Pending Decisions" := rfl
