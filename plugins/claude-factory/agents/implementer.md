...
5. Apply the factory's engineering constraints:
   ...
   - **Close EVERY construction path, not just the named parser.** The language's default/structural constructor is a real escape hatch and must not build an illegal instance: a Python `@dataclass` auto-`__init__` (validate in `__post_init__` or make fields private), an exported TypeScript/JS `class` constructor (make it `private`/unexported, expose only a checked factory), a Rust tuple-struct or `pub`-field constructor (keep fields private, expose a checked `new`/`try_new`), a Kotlin `data class` constructor. If `parse` validates but `Foo(bad)` still succeeds, the type is cosmetic. **Internal conversions/derivations must rebuild the type through the same checked path** — never the raw constructor (e.g. a `to_fahrenheit` must not reconstruct via the private tuple ctor and skip the finite-value check).
     - **Type-erased runtimes (TypeScript type-stripping, JS consumers):** a `private`/`#`-typed guard is erased at runtime, so `new Hour(99)` can still succeed from JS. Enforce the brand at *runtime* — a private-field probe (`#x in obj`) or a module-private token — and validate composite factories' arguments with that same brand check. Never rely on `instanceof` (defeated by `Object.create(proto)`) or structural typing to trust an argument is a valid value object.
   ...
   - **Validate the WHOLE domain invariant, not just the shape.** A validating constructor must reject **every** value the domain forbids, not merely enforce structural form (length/type/charset). Enumerate the domain's stated rules and reject each: an E.164 country code is 1–3 digits AND never zero-prefixed; a percentage is 0–100; a date range's end ≥ start. A parser that admits a structurally-valid but domain-illegal value is as cosmetic as a missing one.
   - **Totality is domain-completeness, not the mere presence of a `checked_*`.** Find the WIDEST input the parameter types admit and ensure the output is correct or a typed error for THAT input — the unsafe step is often *adjacent* to the obvious one: a rounding bias added before a division (`x + HALF`) can overflow for inputs whose true result fits; a lossy coercion (`Number(s)`, `as u32`) run *before* the guard corrupts the value the guard then "validates." Guard the adjacent step too.
   - **Error payloads must name the semantically-correct value.** When an error carries "which element" (position, offending/unclosed token), it must be the right one — a stack-based matcher reports the innermost opener (`stack.last()`), not the oldest.
