...
- **Domain-completeness of validation.** A validating constructor must reject every value the domain forbids, not just enforce structural shape. Enumerate the type's stated domain invariants (e.g. "E.164 country code is never zero-prefixed", "percentage is 0–100") and confirm each illegal-but-structurally-valid value is rejected. A parser that admits a structurally-valid but domain-illegal value is cosmetic — veto.
...
- **Totality of public boundaries — required even for untested input paths, and judged by domain-completeness not the mere presence of a guard.** A public function must not panic or silently wrap on any input its parameter types admit. If an arithmetic/indexing/unwrap on a publicly-constructable input can panic or wrap (e.g. `u64` subtraction when the type permits `amount > balance`), veto — **even if no current test drives that input.** Crucially, check the WIDEST admissible input, and the steps *adjacent* to the obvious operation: a rounding bias added before a division (`x + HALF`) that overflows for representable inputs, or a lossy coercion (`Number(s)`, `as u32`) run *before* the guard so the guard validates an already-corrupted value, are both totality failures — veto. Demand checked arithmetic returning the typed error, or a parameter type that makes the bad input unconstructable. This is NOT "code ahead of tests" / gold-plating (item 5) — panic-safety is exempt from the narrowest-change exclusion; do not veto a guard under item 5 for lacking a test, and do not let its removal create a panicking path.
...
- **Error-payload correctness:** for an error variant carrying "which element" (a position, the offending/unclosed token, an unmatched item), trace one nested/multi-candidate input by hand and confirm the reported element is the semantically-correct one — a stack-based matcher must report the innermost opener (top of stack), not the oldest. A correctly-typed error naming the wrong value is a defect — veto.
